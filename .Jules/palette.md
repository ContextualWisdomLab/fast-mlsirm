...
## 2024-10-24 - Accessibility changes must not conflict with contrast preservation
**Learning:** Adding CSS like `tbody:hover tr:not(:hover) { opacity: 0.5; }` to dim inactive rows was rejected because it reduces adjacent data contrast, conflicting with a prior contrast-preservation decision (#550). Removing the focus outline (`main:focus { outline: none; }`) was also rejected as it suppresses the generic focus indicator. Also, do not mix unrelated semantic changes (like adding `role="status"` to empty states) in the same PR as styling changes.
**Action:** When proposing styling changes, respect existing contrast-preservation policies. Ensure each UX/accessibility improvement is submitted as a single, narrowly focused PR.
