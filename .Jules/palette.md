...
## 2024-08-16 - Single-Click Copy for Export Blocks
**Learning:** For sections designed for data export (like JSON/CSV contained within `.export-block pre` elements), users often struggle to manually highlight all content for copying. Adding `user-select: all;` enables a single-click selection, significantly improving the UX for data exfiltration without requiring complex JS copy buttons.
**Action:** Always apply `user-select: all;` to the CSS of containers strictly intended for bulk copying (e.g., `<pre>` tags containing raw export payloads) to facilitate effortless copying.
