...
## 2024-08-11 - user-select: all on Export Blocks
**Learning:** Adding `user-select: all` to `<pre>` tags globally can break user ability to highlight specific words. However, for specific containers strictly designed for bulk-copying data (like exported JSON or CSV content), applying this property enables single-click copying, significantly improving user experience for these specific scenarios.
**Action:** When creating UI elements intended exclusively for bulk copying, apply the `user-select: all;` CSS property, but ensure the CSS rule is strictly scoped to specific export containers (e.g., `.export-block pre`) rather than applied globally.
