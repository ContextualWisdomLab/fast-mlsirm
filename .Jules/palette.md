...
**Learning:** Native `<summary>` elements can receive a browser focus outline after pointer activation, but a blanket `:focus { outline: none; }` rule can erase the keyboard fallback in user agents that do not restore `:focus-visible` as expected.
**Action:** Suppress only pointer-like focus with `:focus:not(:focus-visible) { outline: none; }` and retain an explicit visible `:focus-visible` outline for keyboard navigation; never remove the generic focus outline without a modality-safe fallback.
