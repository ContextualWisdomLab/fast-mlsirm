...
## 2023-10-27 - Empty State ARIA Atomic Constraints
**Learning:** In fast-mlsirm HTML reports, empty state components must use `aria-atomic="true"` alongside `role="status"` to ensure screen readers announce the full message context. However, modifying these templates requires updating exact HTML string assertions in tests like `test_scoring_essay_report_html.py` and `test_scoring_essay_facets_report_html.py`.
**Action:** When updating HTML templates, always proactively search the test suite for exact string matches of the modified elements (e.g., `<div class="empty-state"`) and update their assertions to match the new markup before pushing.
