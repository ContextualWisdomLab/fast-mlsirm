...
## 2025-05-18 - Fail-Safe Focus Indication for Summaries
**Learning:** Suppressing mouse focus rings using `:focus { outline: none; }` strips fallback focus from browsers that do not support `:focus-visible`.
**Action:** Use `:focus:not(:focus-visible) { outline: none; }` to fail closed and preserve native focus rings when custom `:focus-visible` outlines cannot be rendered.
