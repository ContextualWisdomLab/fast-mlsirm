...
## 2026-08-11 - Replace Hardcoded System Colors in Reports
**Learning:** Hardcoding CSS system colors like `GrayText` in HTML reports overrides intended design system colors, especially for empty states and borders, degrading UI consistency and contrast management. The codebase uses CSS variables like `var(--muted)` and `var(--line)` to manage these styles correctly across light and dark modes.
**Action:** Use existing contrast-managed CSS variables (e.g. `var(--muted)`, `var(--line)`) instead of hardcoded system colors when styling `.empty-state` elements or component borders in HTML reports.
