...
## 2026-08-05 - Suppress Default Focus Outline for Mouse Users\n**Learning:** Relying solely on `:focus-visible` to style focus outlines is insufficient because browsers still apply their default, jarring outline on standard `:focus` (e.g., when a user clicks an element like `<summary>` or `<pre>` with a mouse). Keyboard-accessible elements can appear visually unpolished for pointing-device users.\n**Action:** When adding `:focus-visible` styles to generic focusable containers or interactive elements in HTML reports, always explicitly remove the default outline using `:focus { outline: none; }` to maintain a smooth experience for mouse clicks.
