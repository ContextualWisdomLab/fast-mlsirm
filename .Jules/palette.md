...
## 2024-09-20 - Skip Link Keyboard Accessibility
**Learning:** Skip-to-content links require their target container (like `<main>`) to have `tabindex="-1"`. Without this, programmatic focus via keyboard navigation won't work correctly across all browsers, leaving keyboard users stranded at the top of the page.
**Action:** Always verify that structural containers targeted by skip links include `tabindex="-1"` and an appropriate focus style reset like `:focus:not(:focus-visible) { outline: none; }` to prevent unintended mouse click outlines.
