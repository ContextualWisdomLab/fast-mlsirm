...
## 2026-08-30 - Separate Muted Text and Border Palette Tokens
**Learning:** Replacing `GrayText` with one `--muted` token fixes hardcoded system-color drift, but using the same token for both secondary text and component borders couples two distinct contrast roles. A dedicated `--line` token keeps borders independently tunable across light, dark, and forced-white print canvases. Token separation alone is not sufficient: component boundaries must retain at least 3:1 non-text contrast against the canvas where they are rendered.
**Action:** Use `var(--muted)` for secondary/empty-state text and `var(--line)` for section, table, and code-block borders. Define `--line` so WCAG relative-luminance regression evidence proves at least 3:1 contrast against the light white canvas, the dark black canvas, and the forced-white print canvas; reset the print token explicitly whenever print forces white. Keep `--muted` and review-accent print resets independent from this border contract.
