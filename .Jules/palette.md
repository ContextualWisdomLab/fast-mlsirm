...
## 2026-08-11 - Do Not Use Hardcoded Colors in Themed Components
**Learning:** Hardcoding colors (e.g., `white`) on elements that use thematic CSS variables for their background (e.g., `var(--teal)`) leads to contrast failures in dark mode or custom themes where `--bg` and text colors swap roles.
**Action:** Always use paired thematic variables (e.g., `color: var(--bg)`) rather than hardcoded colors for elements adapting to dark/light modes.
