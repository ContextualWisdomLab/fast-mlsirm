...
## 2026-08-11 - Scoped Text Selection for Code Blocks
**Learning:** While applying `user-select: all;` to `<pre>` blocks containing JSON or CSV exports is a great UX improvement for bulk copying, applying it globally to all `<pre>` tags (or accidentally to table rows like `tbody tr:hover`) causes a major UX regression. It prevents users from highlighting and copying a small, specific portion of the text (e.g., a single sentence or error line) inside general code blocks.
**Action:** When adding `user-select: all;` to enable single-click bulk copying, ensure the CSS rule is strictly scoped to the specific export containers (e.g., `.export-block pre`) rather than applied globally.
