...
## 2025-02-23 - Table Body `<th scope="row">`, Tabular Numbers, and Focus Outline Cleanup\n**Learning:** In the item bank lifecycle report, the skip link target `<main>` lacked the `tabindex="-1"` required for keyboard accessibility, and the timeline table was missing `<th scope="row">` for the first column, harming screen reader context. Additionally, missing `font-variant-numeric: tabular-nums;` on the `body` hindered readability for numerical data.\n**Action:** Always add `tabindex="-1"` and `main:focus:not(:focus-visible){outline:none;}` to primary `<main>` containers in reports. Use `<th scope="row">` instead of `<td>` for identifying columns in tables, and apply tabular-nums to body styles.
