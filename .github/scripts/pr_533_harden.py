"""Apply and verify the one-shot PR 533 provider-boundary hardening patch."""
...
TESTS = Path("tests/test_scoring_enterprise_explicit_values_provider_boundary.py")


def _replace_exact(text: str, old: str, new: str, label: str) -> str:
    """Replace one exact block or fail without modifying an unexpected head."""
    if text.count(old) != 1:
...
def _implementation_text() -> str:
    """Return the hardened implementation derived from the exact branch head."""
    text = IMPLEMENTATION.read_text(encoding="utf-8")
    text = _replace_exact(
        text,
        "    AssessmentSpecError,\n    CanonicalContract,",
        "    CanonicalContract,",
        "AssessmentSpecError import",
    )
    text = _replace_exact(
        text,
        textwrap.dedent(
        ...
        "post-collection record limit",
    )

    candidates_start = text.index(
        "    def _candidates(self, source_text: str) -> tuple[_Candidate, ...]:"
    )
    candidates_end = text.index(
        "    @staticmethod\n    def _deadlines",
        candidates_start,
    )
    candidates_block = textwrap.indent(
        textwrap.dedent(
            '''\
    ...
    text = text[:candidates_start] + candidates_block + text[candidates_end:]

    output_start = text.index("def _validated_parser_output(")
    output_end = text.index("def parse_enterprise_explicit_values(", output_start)
    ...
    callback_start = text.index(
        ...
        text.index("def parse_enterprise_explicit_values("),
    )
    callback_end = text.index(
        "    return _validated_parser_output(source_record, source_text, values)\n",
        callback_start,
    ) + len("    return _validated_parser_output(source_record, source_text, values)\n")
    callback_block = textwrap.dedent(
        '''\
            if parser is None:
                values = resolved.parse(source_record, source_text)
            else:
    ...
    return text[:callback_start] + callback_block + text[callback_end:]


def _test_text() -> str:
    """Return focused security and ownership regression tests."""
...
def main() -> None:
    """Write the hardened implementation and focused regression tests."""
    IMPLEMENTATION.write_text(_implementation_text(), encoding="utf-8")
    TESTS.write_text(_test_text(), encoding="utf-8")
