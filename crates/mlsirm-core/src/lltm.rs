...
fn validate(
    ...
    let n_q = crate::checked_mul_usize(n_items, n_basic, "n_items * n_basic overflows usize")?;
    if q_design.len() != n_q {
    ...
    if m > n_items {
        return Err(format!(
            "design has more columns ({m}) than items ({n_items}); eta not identified"
        ));
    }
    for a in 0..m {
    ...
    if !gram_full_rank(&mut gram, m, 1e-9 * maxg.max(1e-300)) {
        return Err(
            "design matrix (with intercept) is column-rank-deficient; eta is not identified".into(),
        );
    }
    Ok(())
}
...
fn ls_project(design: &[f64], m: usize, n_items: usize, b_init: &[f64]) -> Vec<f64> {
    ...
    match solve_small_checked(gram, rhs) {
...
fn run_em_lltm(
    ...
    for _ in 0..cfg.max_iter {
        ...
        for p in 0..n_persons {
            total_ll += person_posterior(
                p, y, observed, n_items, q, &log_w, &log_p1, &log_p0, &mut post,
            );
            for i in 0..n_items {
                let idx = p * n_items + i;
                if observed[idx] {
