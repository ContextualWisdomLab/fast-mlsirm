...
pub fn rudner_classification(
    ...
    // Guard against usize overflow before assemble allocates m * n entries.
    cutscores
        .len()
        .checked_mul(n)
        .ok_or_else(|| "cutscores.len() * n overflows".to_string())?;
...
pub fn lee_classification(
    ...
    cutscores
        ...
        .ok_or_else(|| "cutscores.len() * n_points overflows".to_string())?;

    // f(x | theta_i): lord_wingersky treats each point as one "node"; feed it
    // per point so the table stays (n_items + 1) x 1.
    let sc = n_items
        .checked_add(1)
        .ok_or_else(|| "n_items + 1 overflows".to_string())?;
    let dist_len = n_points
        .checked_mul(sc)
        .ok_or_else(|| "n_points * (n_items + 1) overflows".to_string())?;
