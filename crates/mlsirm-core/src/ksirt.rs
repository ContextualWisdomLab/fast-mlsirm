    ...
    for j in 0..k {
        ...
        let option_index: Vec<usize> = x
            ...
            .map(|(i, row)| {
                ...
                options
                    .binary_search_by(|option| {
                        option.partial_cmp(&value).expect(
                            "internal invariant violation: ksirt compares only finite \
                             responses validated at function entry",
                        )
                    })
                    .unwrap_or_else(|_| {
                        panic!(
                            "option for subject {i} item {j} value {value} not found \
                             in sorted/deduped options (internal invariant violation)"
                        )
                    })
            })
            .collect();
