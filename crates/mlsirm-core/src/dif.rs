...
pub fn logistic_dif(
    ...
    for i in 0..n_items {
        ...
        let sf = |c: f64, df: f64| if c.is_finite() { chi2_sf(c, df) } else { f64::NAN };
...
mod tests {
    ...
    fn bin_ll(k: f64, n: f64) -> f64 {
        ...
        let b = if n - k > 0.0 { (n - k) * (1.0 - p).ln() } else { 0.0 };
        a + b
    }
    ...
    fn logistic_dif_saturated_design_closed_form() {
        ...
        // closed forms
        let ll2: f64 = cells.iter().map(|&(_, _, nn, kk)| bin_ll(kk as f64, nn as f64)).sum();
    ...
    fn logistic_dif_detects_crossing_dif_that_mantel_haenszel_misses() {
        ...
        let c = &lr[cross_item];
        assert!(c.converged);
        assert!(c.p_nonuniform < 0.01, "crossing p_nonuniform {}", c.p_nonuniform);
        assert!(c.p_uniform > 0.05, "crossing p_uniform should be n.s.: {}", c.p_uniform);
        assert!(c.flagged_bh, "crossing item not flagged by the omnibus test");
        ...
        assert!(u.p_nonuniform > 0.05, "uniform p_nonuniform should be n.s.: {}", u.p_nonuniform);
        assert!(u.flagged_bh);
        ...
        for (i, r) in lr.iter().enumerate() {
            ...
            if i != cross_item && i != unif_item {
                assert_eq!(r.jg_class, EtsClass::A, "clean item {i} class {:?}", r.jg_class);
            }
        }
    }
    ...
    fn logistic_dif_undefined_on_degenerate_item() {
        ...
        let r0 = &rows[0];
        assert!(!r0.converged, "constant item should not report a converged fit");
        ...
        let cfg_bad = LogisticDifConfig { fdr_q: 0.0, ..LogisticDifConfig::default() };
        assert!(logistic_dif(&y, &group, n, n_items, &cfg_bad).is_err());
        let cfg_it = LogisticDifConfig { max_iter: 0, ..LogisticDifConfig::default() };
