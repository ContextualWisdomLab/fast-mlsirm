...
fn scores_with_lossy_value(value: i64) -> Vec<i64> {
    vec![TWO_POW_53, 0, value, 1, TWO_POW_53 + 2, 2]
}

#[test]
fn direct_rust_rejects_scores_that_change_when_projected_to_binary64() {
    for lossy_value in [TWO_POW_53 + 1, i64::MAX] {
        let x = scores_with_lossy_value(lossy_value);

        let coef_error =
            coef_h(&x, 3, 2).expect_err("lossy score must fail before Mokken arithmetic");
        assert_eq!(
            coef_error,
            "scores must be exactly representable as f64",
            "coef_h must not silently round direct-Rust i64 response evidence: {lossy_value}",
        );
        ...
        assert_eq!(
            aisp_error,
            "scores must be exactly representable as f64",
            "aisp must share the canonical direct-Rust score admission boundary: {lossy_value}",
        );
    }
}
