"""Fail-first relationship contracts for governed RAG perturbation anchors."""
...
def test_anchor_builder_accepts_canonical_requests_not_opaque_fingerprints() -> None:
    """The boundary must inspect the governed pair before storing its identities."""
...
def test_response_perturbation_binds_only_a_changed_response_artifact() -> None:
    """Response kinds preserve query/retrieval provenance and change response content."""
    baseline = _request()
    perturbed = _request(
...
def test_retrieval_perturbation_binds_only_a_changed_retrieval_artifact() -> None:
    """Retrieval kinds preserve query and response artifacts while changing retrieval."""
    baseline = _request()
    perturbed = _request(
    ...
    anchor = build_rag_perturbation_anchor(
        anchor_id="irrelevant_context_anchor",
        baseline_request=baseline,
        perturbed_request=perturbed,
        perturbation_kind=RAGPerturbationKind.IRRELEVANT_CONTEXT,
    )
...
def test_query_perturbation_requires_a_changed_query_revision() -> None:
    """Query kinds preserve the governed task identity while changing its revision."""
    baseline = _request()
    perturbed = _request(
    ...
    anchor = build_rag_perturbation_anchor(
        anchor_id="semantic_query_paraphrase_anchor",
        baseline_request=baseline,
        perturbed_request=perturbed,
        perturbation_kind=RAGPerturbationKind.SEMANTIC_QUERY_PARAPHRASE,
    )
...
def test_unrelated_system_configuration_pair_fails_closed() -> None:
    """A different evaluated system cannot masquerade as one controlled perturbation."""
    with pytest.raises(AssessmentSpecError) as caught:
        build_rag_perturbation_anchor(
            anchor_id="unrelated_system_anchor",
            baseline_request=_request(),
...
def test_response_kind_rejects_a_simultaneous_retrieval_change() -> None:
    """A response perturbation cannot silently include a second changed treatment."""
...
def test_retrieval_kind_rejects_a_simultaneous_response_change() -> None:
    """A retrieval perturbation must keep the scored response artifact fixed."""
    _assert_relationship_error(
        "irrelevant_context",
        _request(
...
def test_construct_basis_distinguishes_sources_from_model_design() -> None:
    """Unsupported literature mappings remain explicit model-design hypotheses."""
    ...
    literature_aligned = build_rag_perturbation_anchor(
        anchor_id="grounded_generation_anchor",
        baseline_request=_request(),
        perturbed_request=_request(
    ...
    model_design = build_rag_perturbation_anchor(
        anchor_id="citation_hypothesis_anchor",
        baseline_request=_request(),
        perturbed_request=_request(
        ...
        perturbation_kind="citation_target_swap",
    )
