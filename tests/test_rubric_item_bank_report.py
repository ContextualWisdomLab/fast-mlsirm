...
def test_html_report_is_standalone_accessible_and_escapes_title() -> None:
    ...
    # The skip-link target needs tabindex="-1" to receive focus at all, which
    # then draws an outline on mouse click; suppressing that is correct only
    # for that element. An unscoped rule would strip the mouse-focus outline
    # from every control in the report, so pin the count as well as the rule.
