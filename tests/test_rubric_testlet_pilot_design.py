...
def _testlet_records():
    """Return binary records spanning one multi-item and one singleton testlet."""
...
def test_testlet_design_is_deterministic_and_discloses_bundle_structure():
    """Testlet mappings, singleton status, and provenance remain fingerprinted."""
    ...
    assert first.testlet_ids == (
        "query_testlet_alpha",
        "query_testlet_beta",
    )
    assert first.item_testlet_ids == (0, 0, 1)
    assert first.testlet_item_counts == (2, 1)
    assert first.multi_item_testlet_ids == ("query_testlet_alpha",)
    assert first.singleton_testlet_ids == ("query_testlet_beta",)
    assert first.responses == ((1, 0, None), (0, 1, 1))
    ...
    assert payload["testlet_item_counts"] == [2, 1]
    assert payload["singleton_testlet_ids"] == ["query_testlet_beta"]
...
def test_testlet_fit_kwargs_match_the_existing_rust_backed_api():
    """The handoff emits fresh response and testlet arrays accepted by fit_testlet."""
...
def test_testlet_design_is_factory_sealed_and_schema_bound():
...
def test_testlet_builder_reuses_binary_fail_closed_validation_and_exports():
    """No weaker response parser, aggregation, category, or export path is added."""
    ...
    pilot = _pilot()
    duplicate_cell = (
        _observation(pilot, rater_id="rater_alpha", category=1),
        _observation(pilot, rater_id="rater_beta", category=0),
    )
    with pytest.raises(PilotObservationError) as duplicate:
        build_testlet_pilot_design(duplicate_cell)
    assert duplicate.value.code == "duplicate_person_item_cell"
