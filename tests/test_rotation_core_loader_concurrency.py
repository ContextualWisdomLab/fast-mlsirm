"""Concurrency regression tests for the secondary rotation extension loader."""
...
import fast_mlsirm
import fast_mlsirm._rotation_core_loader as loader


def test_rotation_core_does_not_return_partially_initialized_module(monkeypatch, tmp_path):
    ...
    try:
        with ThreadPoolExecutor(max_workers=2) as pool:
            first = pool.submit(loader.rotation_core)
            assert exec_started.wait(timeout=1.0)
            second = pool.submit(loader.rotation_core)
            time.sleep(0.05)
            assert not second.done(), (
                "rotation_core returned the sys.modules entry while the first "
