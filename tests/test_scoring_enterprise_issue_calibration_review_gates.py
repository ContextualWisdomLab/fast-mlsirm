...
def _managed_observation_metadata(issue, references) -> dict[str, object]:
    """Return exact package-managed metadata for an adversarial observation."""
    return {
        ...
        "enterprise_supporting_evidence_count": sum(
            value.evidence_role is EvidenceRole.SUPPORTING for value in references
        ),
        "enterprise_counter_evidence_count": sum(
            value.evidence_role is EvidenceRole.COUNTER for value in references
        ),
        "enterprise_context_evidence_count": sum(
            value.evidence_role is EvidenceRole.CONTEXT for value in references
        ),
    }
