...
fn ari(a: &[u32], b: &[u32]) -> f64 {
    ...
    let sum_a: f64 = (0..ka)
        .map(|i| c2((0..kb).map(|j| tab[i * kb + j]).sum()))
        .sum();
    let sum_b: f64 = (0..kb)
        .map(|j| c2((0..ka).map(|i| tab[i * kb + j]).sum()))
        .sum();
    let n = a.len() as u64;
...
fn mixture_c1_equals_fit_mmle_2pl() {
    ...
    assert!(
        rmse(&mix.a, &mmle.a) < 1e-12,
        "a RMSE {}",
        rmse(&mix.a, &mmle.a)
    );
    assert!(
        rmse(&mix.b, &mmle.b) < 1e-12,
        "b RMSE {}",
        rmse(&mix.b, &mmle.b)
    );
...
fn recovers_mixed_rasch_c2() {
    ...
    let pi_true = 0.6;
    let b0: Vec<f64> = (0..j)
        .map(|i| -2.0 + 4.0 * i as f64 / (j - 1) as f64)
        .collect();
    let a0 = vec![1.0f64; j];
    ...
    assert!(
        ari(&res.map_class, &cls) > 0.35,
        "ARI {}",
        ari(&res.map_class, &cls)
    );
}
...
fn mixture_validate_rejects_nonfinite_optimizer_config() {
    ...
    let d = MixtureConfig::default();
    let bad =
        |cfg: &MixtureConfig| fit_mixture(&y, &obs, 4, 3, 2, MixtureModel::Rasch, cfg).is_err();

    assert!(bad(&MixtureConfig {
        ridge_a: f64::NAN,
        ..d
    }));
    assert!(bad(&MixtureConfig { ridge_b: -1.0, ..d }));
    assert!(bad(&MixtureConfig {
        start_spread: f64::INFINITY,
        ..d
    }));
}

#[test]
fn mixture_private_singular_updates_and_empty_initialization_are_safe() {
...
fn mixture_validate_rejects_dimension_overflow() {
    ...
    assert!(fit_mixture(&y, &obs, 1, 2, usize::MAX, MixtureModel::Rasch, &cfg,).is_err());
}
