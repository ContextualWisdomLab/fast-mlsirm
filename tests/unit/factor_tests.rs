...
fn glbfa_error_paths() {
    ...
    // Here off-diagonals a,b,c are chosen to satisfy a+b+c = -1.5, so for p=3:
    // sum(R) = 3 + 2(a+b+c) = 0 exactly.
    let a: f64 = -0.15885683833831;
    let b: f64 = -0.4821664994140733;
    let c: f64 = -0.8589766622476167;
    assert!(((a + b + c) + 1.5_f64).abs() < 1e-12);
    #[rustfmt::skip]
    let zero_sum: [f64; 9] = [
        1.0, a, b,
        a, 1.0, c,
