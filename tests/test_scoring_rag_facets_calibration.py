...
def test_rag_facets_bundle_delegates_to_shared_many_facet_design() -> None:
    ...
    assert all(design.respondent_ids == ("retrieval_stack_a_run_001",) for design in bundle.designs)
    assert all(design.task_ids == ("refund_policy_query",) for design in bundle.designs)
    assert all(design.rater_engine_ids == ("fixture_engine",) for design in bundle.designs)
