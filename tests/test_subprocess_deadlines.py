"""Contracts for operation-specific bounded subprocess execution."""
...
def test_operation_defaults_distinguish_metadata_inventory_and_long_studies() -> None:
    ...
    metadata = deadlines.resolve_timeout_seconds(
        deadlines.SubprocessOperation.CARGO_METADATA, {}
    )
    inventory = deadlines.resolve_timeout_seconds(
        deadlines.SubprocessOperation.CARGO_TEST_LIST, {}
    )
    study = deadlines.resolve_timeout_seconds(
        deadlines.SubprocessOperation.STATISTICAL_TEST, {}
    )
...
class _FakeProcess:
    ...
    def communicate(self, timeout=None):
        ...
        if self._timeout_once and len(self.communicate_calls) == 1:
...
def test_successful_run_uses_isolated_process_group_and_returns_completed_process(
    monkeypatch,
) -> None:
...
def test_process_group_probe_treats_permission_denial_as_still_alive(monkeypatch) -> None:
    """EPERM fails closed rather than being mistaken for group disappearance."""
    monkeypatch.setattr(
        deadlines.os,
        "killpg",
