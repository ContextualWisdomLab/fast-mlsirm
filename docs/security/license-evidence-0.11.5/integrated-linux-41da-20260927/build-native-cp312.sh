...
test ! -e "$D/dist-reviewed-cp312"
mkdir -p "$D/dist-reviewed-cp312" "$D/logs-reviewed-cp312" "$D/home"
...
docker run --platform linux/arm64 --pull=never --network=none --rm --memory=5g \
  ...
  "$IMG" bash -euo pipefail -c '
    ...
    ldd --version | sed -n 1p > /w/logs-reviewed-cp312/toolchain.txt
    rustc --version >> /w/logs-reviewed-cp312/toolchain.txt
    maturin --version >> /w/logs-reviewed-cp312/toolchain.txt
    cd /w/arm-src
    maturin build --release --offline --locked --compatibility manylinux2014 \
      -i /opt/python/cp312-cp312/bin/python -o /w/dist-reviewed-cp312 \
      > /w/logs-reviewed-cp312/core-build.log 2>&1
    auditwheel show /w/dist-reviewed-cp312/fast_mlsirm-*.whl \
      > /w/logs-reviewed-cp312/auditwheel.log 2>&1
  '
shasum -a 256 "$D"/dist-reviewed-cp312/*.whl "$D"/logs-reviewed-cp312/*
