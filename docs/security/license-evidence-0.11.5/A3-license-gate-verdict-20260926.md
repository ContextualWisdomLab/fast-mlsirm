...
Run the real-wheel hash regression explicitly with `python tests/test_license_inventory_generator.py WHEEL EXACT_SOURCE_ROOT`; the default pytest collection needs no wheel and has no skip. At the correction head, the default SPDX/inventory set passed 236/236; the explicit actual-wheel check passed, and a copy with a changed source `NOTICE` was rejected. The source archive identifies #2157 head `58b7b23f`. A later [integration candidate](nonlinux-upstream-20260926/REVIEW.md#integrated-source-candidate) resolves the `uv.lock` conflict; it still needs its own built wheels and hosted checks.
