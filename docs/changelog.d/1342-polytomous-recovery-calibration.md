...
- Extend deterministic GRM, GPCM, and fixed-item-parameter calibration recovery studies to require signed-bias, MAE, finite positive EAP posterior uncertainty, and empirical coverage of the normal-approximation interval `theta_eap ± 1.96 * theta_sd` alongside RMSE, while retaining correlation only as supplementary recovery evidence.
