...
- Replay deterministic Rust-owned output domains before public marshalling: floored `tau2` and posterior variances must be non-negative, shrinkage weights and ETS category probabilities must remain in `[0, 1]`, while finite negative `tau2_raw` remains valid as the documented pre-floor diagnostic.
