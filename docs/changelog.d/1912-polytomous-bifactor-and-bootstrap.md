...
- Fail closed when polytomous bifactor Oakes standard errors are requested. The current Rust core does not yet expose the category-count derivatives required by Oakes' identity; callers may explicitly run point estimation with `compute_oakes_se=False`, but no uncertainty claim is emitted.
