...
- Bound KSIRT Nadaraya–Watson evaluation work to 20,000,000 person × item × grid terms in both the Python admission boundary and the Rust core. The shape-only preflight runs before response-value materialization/traversal, so a small response matrix combined with an extreme evaluation grid cannot trigger unbounded OCC work or allocation; the Rust check uses overflow-safe multiplication and remains authoritative for direct crate callers.
