...
- D-study control containers are now admitted only as exact built-in list/tuple values before iteration or pair unpacking, so caller-defined sequence and pair callbacks cannot run while `n_i_prime` / `n_prime` semantics are being established; ordinary list/tuple controls and concrete Python/NumPy integer entries remain supported.
