...
- Reject a self-referential or otherwise cyclic list/tuple response or cluster (for example `a = []; a.append(a)`) before flattening, instead of looping until the process is killed; the admission-hardening stack walk now tracks each container's identity and fails closed the moment one reappears.
