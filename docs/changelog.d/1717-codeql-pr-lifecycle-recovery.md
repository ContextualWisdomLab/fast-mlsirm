...
Cross-revision acceptance RED `b2c3a792aadc7cce08b0e57db364fbdfdfd2ff8f` proves that a buyer packet could previously accept a valid-looking acceptance summary from a different source revision and then stamp the packet with the current repository `HEAD`. Causal GREEN `67cbf6d514c446f1826a8aefed23e0ee2cb788aa` captures the packet source identity once, requires the acceptance summary to carry that exact sealed commit, and reuses the same identity in the emitted manifest.
