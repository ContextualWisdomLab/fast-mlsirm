...
Optional-evidence source RED `a46bd714ee86b2f076eaa4a8107c0b9ae5d2f29d` proves that a successful benchmark or release-evidence manifest carrying an explicit source revision could disagree with the buyer packet's exact repository revision and still be admitted. Causal GREEN `d9e42d9a63188f63aa392a03cd698f8b2f2d2e0c` rejects either contradictory identity before collecting optional evidence. The guard remains backward-compatible with manually authored legacy optional manifests that omit `source_commit`; canonical benchmark and release-index builders already emit that identity, so canonical cross-revision evidence fails closed.
