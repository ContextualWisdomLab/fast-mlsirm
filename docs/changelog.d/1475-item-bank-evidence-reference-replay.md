...
- Preserve the stable `ItemBankLifecycleError` replay boundary when a record contains a post-construction-tampered embedded evidence reference, so report/record callers do not receive an uncaught raw `ValueError` from nested provenance validation.
