...
- Bound the worst-case contiguous credible-set search to 20,000,000 candidate additions with checked triangular-work arithmetic before level-array allocation, so unusually large ordered scales fail closed instead of inducing unbounded quadratic CPU work.
- Add fail-closed validation for empty or non-finite draws, malformed weights, non-increasing cut scores, invalid credible mass, excessive credible-set search work, and non-finite posterior moments, plus joint draw/weight permutation regressions.
