...
- Keep the self-contained report fail-closed under Content Security Policy by authorizing only the exact inline stylesheet bytes through a SHA-256 `style-src` hash, denying scripts and unsafe inline styles, and denying hostile embedding with `frame-ancestors 'none'`.
- Add regression evidence that the rendered stylesheet hash matches the emitted CSP, that anti-framing remains present, and that the skip-link/focus contracts remain present.
...
- W3C, *Content Security Policy Level 3*, https://www.w3.org/TR/CSP3/ — defines hash-source matching for inline style content and the `frame-ancestors` directive governing which ancestors may embed a protected resource; the report authorizes the SHA-256 digest of the exact `_css()` UTF-8 bytes emitted inside its single `<style>` element and permits no framing ancestors.
