...
- Return fresh canonical `FitCapability` value objects and validate direct construction against the same sealed model-estimator rows used by the manifest, preventing caller mutation or forged frozen records from redefining versioned capability authority.
- Make the exported `FitConfig` model, estimator, and optimizer vocabularies immutable so callers cannot mutate validation authority before the capability module is imported or before a fit is admitted.
