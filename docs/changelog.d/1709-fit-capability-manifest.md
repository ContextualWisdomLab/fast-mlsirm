...
- Return fresh canonical `FitCapability` value objects and validate direct construction against the same sealed model-estimator rows used by the manifest, preventing caller mutation, forged frozen records, or later mutation of config-set aliases from redefining versioned capability authority.
