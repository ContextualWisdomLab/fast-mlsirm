...
**Architecture:** A new `mlsirm_core::bifactor_indices` module accepts a standardized item-by-factor loading matrix, item uniquenesses, an explicit general-factor index, and a structural-zero tolerance. It computes ECV variants, item ECV, PUC when the loading pattern is a strict bifactor pattern, omega total, omega hierarchical, and construct replicability H. Integration tests pin the formulas to the published CRAN calculator example; documentation explicitly separates score interpretability from model-selection evidence.
...
- The API requires standardized orthogonal-factor loadings and explicit uniquenesses; it does not silently convert raw IRT slopes.
...
**Interfaces:**
- Consumes: future `mlsirm_core::bifactor_indices::{bifactor_indices, BifactorIndicesConfig}`.
- Produces: executable formula, validation, and strict-pattern expectations for the Rust implementation.
...
Cover non-strict cross-loadings (`puc == None`), zero-tolerance handling, invalid dimensions, out-of-range general factor, malformed lengths, empty items/factors, non-finite values, negative uniquenesses, and invalid standardized loadings.
...
**Interfaces:**
- Consumes: row-major `loadings: &[f64]`, `uniquenesses: &[f64]`, `BifactorIndicesConfig`.
...
- [ ] **Step 3: Register the module from the crate root**
...
Explain each index, the strict-pattern requirement for PUC, the distinction between model selection and scoreability, and why no universal cutoff is built into the kernel.
...
- [ ] **Step 3: Update literature map and changelog**

Add the Rust module and test paths to the implemented-literature map and describe the new diagnostics under the unreleased changelog section.

- [ ] **Step 4: Commit documentation**

```bash
git add docs/bifactor_scoreability_indices.md docs/papers/implemented-literature-map.md CHANGELOG.md
...
Confirm every output matches the independently calculated example values, every branch is tested, and the docs do not treat scoreability indices as non-nested model-selection tests.

- [ ] **Step 3: Open a draft PR and enable auto-merge only after required checks pass**

```bash
gh pr create --draft --title "feat: add Rust bifactor scoreability indices" --body-file /tmp/pr-body.md
gh pr checks --watch
gh pr ready
