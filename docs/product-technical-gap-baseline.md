...
## 24. Item-bank report skip-link design assurance

Status: **Proposed**  
Tracked change: [PR #2025](https://github.com/ContextualWisdomLab/fast-mlsirm/pull/2025)

### PRD / TRD boundary

The source-text-free item-bank HTML report must let keyboard and assistive-technology
users bypass repeated content and arrive at the `main` landmark with a visible,
deterministic focus indicator. The presentation focus target does not change item-bank
domain records, psychometric evidence, scoring, or lifecycle truth.

```text
User -> Skip to report link -> #main-content (tabindex=-1)
Browser -> visible focus indicator -> semantic report sections/tables
```

ERD impact: none. No aggregate, entity, relation, column, or persistence contract changes.

### Gap / action / status

| Gap | Exact evidence | Action | Status |
| --- | --- | --- | --- |
| Skip target was not programmatically focusable | Original #2025 source delta | Preserve `tabindex="-1"` on `main` | Source candidate |
| A new `:focus:not(:focus-visible){outline:none}` exception could hide the destination indicator according to UA/modality behavior | Head `db02aba2fb16ec9eafb7652c4b65d30aa4cb10de` | Delete the suppression in `fcd5274ba37da3171437ea58acee32b12c2391fb` | Source repaired |
| The regression asserted the suppressor itself and allowed whitespace variants | Predecessor test at `db02aba2...` | Reject normalized `outline:none` in `423f0ca539893140fa89c5597a88144e247eb901` | Contract candidate |
| Current-head interaction evidence is absent | No repository-pinned browser execution artifact | Verify Tab/Enter, pointer, touch, focus destination and visible indicator in Chromium/Firefox/WebKit plus AT | Pending |
| Responsive, locale, and performance evidence is absent | No current-head 320/768/desktop screenshots, eight-locale matrix, or render/interaction median·p95 | Produce exact-head evidence without reduced samples | Pending |
| #2025 also contains two non-UI files overlapping #2015 but with different blobs | `dif.py` and `tools/inventory_public_api.py` blob comparison against #2015 | Preserve both deltas; perform ordinary/non-force owner reconciliation before Ready | Pending |

Exact-head required workflows and an independent same-head approval remain merge gates.
Queued, pending, skipped, cancelled, predecessor-head, or source-only evidence is not GREEN.
