...
Observed at: **2026-09-08T04:07:59Z**  
...
| CodeQL rerun recovery, exact-run settlement, and self-repository receipt identity | `ContextualWisdomLab/.github#1902` exact head `1a3133f5efd147bc6892a2920749bf103f4683da`, tree `d08fe5d530a7bd96d59ab190d16960aa9affb3f2`, protected base `main@7fd571dbcdbae6acf29d8f4ee704d7ba6297e4db`, eleven CodeQL-owner paths | Bind every dispatch title and receipt to target/PR/head/base/required-run identity; keep matrix scans `actions: read`; let one non-matrix settlement job own `actions: write`; if status publication is forbidden, accept only a unique exact central run with authenticated workflow/actors/live PR, successful validation and SARIF upload, terminal language gate, and exact unexpired artifact | **Source repaired and Ready for review admission; not merge-authorized.** The current P1's three contracts were RED before implementation: required-run title binding was absent, the handler run name was head-only, and settlement remained inside the matrix. GREEN is 82 focused CodeQL contracts, repository 3,030 passed / 1 skipped / 21 subtests, statement/branch and public-doc coverage 100%, and clean diff. The repair integrates `.github#2032`'s valid least-privilege/single-settlement delta without copying its weaker provenance path; #2032 remains preserved until hosted equivalence is proven. Head is behind 0, mergeable, with zero unresolved threads. A transient Draft event at `2026-09-08T04:07:10Z` was corrected after direct inspection proved an overlapping-output audit artifact rather than a source defect; Ready was restored at `2026-09-08T04:07:47Z` on the unchanged head only for review admission. CodeQL PR `34185891254` is pending; Python Security `34185891146`, Security Scan `34185891248`, and Semgrep `34185891211` are queued, not GREEN. Local actionlint is unavailable; hosted workflow validation and qualifying independent review remain merge gates. Evidence: `.github#1902` comments `5578994491` and correction `5579031179`. |
...
1. Let `.github#1902@1a3133f5` prove its exact required-run-bound receipt or direct-evidence path in
   hosted execution, then repair and prove the separate cross-repository HTTP
   403 publication path tracked by `.github#1929` without weakening identity.
2. Let `.github#1902@1a3133f5` and stacked `.github#1999@64d19495` acquire
