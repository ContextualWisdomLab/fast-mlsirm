...
| CodeQL rerun recovery, exact-run settlement, and self-repository receipt identity | `ContextualWisdomLab/.github#1902` exact head `ccd4dd3344654f91abefcbabbdd06a8fa485e3f2`, tree `125bfb07514d69a75fe82e9a88aef53a4dcd08ad`, protected base `main@7fd571dbcdbae6acf29d8f4ee704d7ba6297e4db`, twelve CodeQL-owner paths | Preserve immutable producer source `S` across a compatible newer default-branch handler `T` only when GitHub proves `S` is `T`'s exact merge-base ancestor; fully validate every same-title handler candidate before requiring exactly one evidence-complete run; keep complete failed-job settlement identity, native pagination, least privilege, SARIF provenance, and exact target/PR/head/base/required-run bindings | **Source repaired and Ready for review admission; not merge-authorized.** RED reproduced the `S != T` lifecycle deadlock and the duplicate-title incomplete-predecessor deadlock. GREEN adds executable receiver/shard/coordinator/settlement ancestry fixtures, divergent-source rejection, incomplete-predecessor plus complete-successor fixtures, and a multiple-complete-candidate rejection. Exact tree: focused two-workflow contracts 101 passed; repository 3,051 passed / 1 skipped / 21 subtests; statement/branch and public-doc coverage 100%; Python compile, Bash execution contracts, and diff check clean. Head is mergeable with zero unresolved threads. Ready was restored at `2026-09-08T06:14:52Z` only for review admission. Fresh exact-head CodeQL PR `34193835240` is pending; Python Security `34193835234`, Security Scan `34193835265`, Semgrep `34193835207`, and Agent Review Runtime `34193748390` are queued, not GREEN. Local actionlint is unavailable; hosted validation and qualifying independent review remain merge gates. Evidence: `.github#1902` comments `5580057010`, `5580192861`, and Ready event `5580200580`. |
...
1. Let `.github#1902@ccd4dd33` prove its exact required-run-bound receipt or direct-evidence path in
...
2. Let `.github#1902@ccd4dd33` and stacked `.github#1999@64d19495` acquire
