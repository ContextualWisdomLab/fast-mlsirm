...
Observed at: **2026-09-08T02:31:20Z**  
...
| CodeQL rerun recovery, exact-run settlement, and self-repository receipt identity | `ContextualWisdomLab/.github#1902` exact head `b9b98cf9ea5cee376156b3c18f2bda9f9bbe9085`, protected base `main@78a4937c684a54ca8e415822c913742f41c6efc4`, ten CodeQL-owner paths | Preserve complete status pagination, live base/head binding, SARIF-before-status publication, one exact failed-run settlement, and the canonical OpenCode App identity; for the self repository only, accept `github-actions[bot]` after exact central run/event/workflow/repository/PR/head/base/App-actor/language/SARIF/publication proof | **Source repaired and Ready for review admission; not merge-authorized.** The current-head P1 proved target-App HTTP 403 followed by self `github-token` publication whose creator the consumer rejected. RED covered the app-403/bot-success path, unrelated creator, cross-repository bot, forged run/workflow/title/actor, missing evidence, and wrong handler-run receipt. GREEN is 76 focused production-shell tests normally and with `GITHUB_ACTIONS=true -W error`, repository 3,024 passed / 1 skipped / 21 subtests, statement/branch and public-doc coverage 100%, and clean diff. Head is behind 0, mergeable, with zero unresolved threads. Ready was restored at `2026-09-08T02:31:08Z` only for review admission; CodeQL PR `34180332107`, Python Security `34180332090`, Security Scan `34180332087`, and Semgrep `34180332079` are queued, not GREEN. Local actionlint is unavailable; hosted workflow validation and qualifying independent review remain merge gates. Evidence: `.github#1902` comment `5578219362`. |
...
1. Let `.github#1902@b9b98cf9` prove the bounded self-repository receipt path in
...
2. Let `.github#1902@b9b98cf9` and stacked `.github#1999@64d19495` acquire
   exact-head hosted checks and independent review; neither Ready transition is
   approval or merge authority, and no auto-merge change is recorded here.
