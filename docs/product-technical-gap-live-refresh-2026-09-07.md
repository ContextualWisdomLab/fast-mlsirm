...
Observed at: **`2026-09-08T08:57:43Z`**  
...
| CodeQL rerun recovery, live-base liveness, exact-run settlement, and receipt identity | `ContextualWisdomLab/.github#1902` exact head `ebf054e67f7dbeb7c9b8d0f90e2bc1c151471a2b`, tree `03a9ebd2ffc3504e52028499402b703394b5430e`, protected base `main@7fd571dbcdbae6acf29d8f4ee704d7ba6297e4db`, twelve CodeQL-owner paths | Capture one validated live base before matrix expansion; if that attempt snapshot becomes stale on the same base ref, bind a new attempt to the verified live base, require the complete terminal language-job map, and request one whole-run rerun of the exact required run; retain failed-only recovery on an unchanged base and require exactly one state-consistent language gate plus preserved SARIF in every receipt consumer | **Source repaired and Ready for review admission; hosted gates pending.** Exact head `ebf054e6…` is an ordinary non-force child of concurrent gate-proof head `7ca416ee…`, preserving its valid predecessor-gate delta. RED reproduced 13 base-recovery/rerun-mode/gate-proof failures plus one separate coordinator gate mismatch. GREEN: two-workflow contracts 129 passed; repository 3,079 passed / 1 skipped / 21 subtests; statement `13,194/13,194`, branch `5,330/5,330`, total and public-doc coverage 100%; compileall and diff check clean. Protected-base comparison is 55 ahead / 0 behind, mergeable, with zero unresolved threads. Ready was restored at `2026-09-08T08:57:34Z` only for review admission. Python Security `34207344460` and Security Scan `34207344467` are queued; Semgrep `34207344436` and CodeQL PR `34207344514` are pending; Agent Review Runtime Quality `34207159185` is queued. None is GREEN. Hosted checks and qualifying independent current-head review remain merge gates; local actionlint is unavailable. Evidence: `.github#1902` comments `5582176132` and `5582198291`. |
...
1. Let `.github#1902@ebf054e6` prove its base-advance whole-attempt recovery and exact required-run-bound receipt/direct-evidence path in hosted execution, then repair and prove the separate cross-repository HTTP
   403 publication path tracked by `.github#1929` without weakening identity.
2. Let `.github#1902@ebf054e6` and stacked `.github#1999@64d19495` acquire
