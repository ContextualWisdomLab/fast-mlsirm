...
| TEPP posterior topic-context influence | PRD-FR-063, TRD-MLT | ADR-0022 | `mlsirm_core::topic_context_influence` validates the exact posterior, Event Lineage, temporal provenance, and BU/PU/team/person membership contract; full-data draws return `CaseDeletionRefitEvidenceUnavailable` because v1 has no deleted-data refit or per-case likelihood evidence | Proposed / fail-closed prerequisite |
