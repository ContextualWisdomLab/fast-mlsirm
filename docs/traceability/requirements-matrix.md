...
| TEPP-anchored Event Lineage channel weights | PRD-FR-063, TRD-MLT-006 | ADR-0023 | Rust admits continuous pair/channel evidence and exact accepted TEPP v1 identity, then returns `IndependentCriterionObservationsUnavailable`; no floor, Python fit, or local normalization | Proposed / pair-level producer evidence missing |
