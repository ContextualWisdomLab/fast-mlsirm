...
| TEPP posterior topic-context influence | PRD-FR-064, TRD-MLT-007..010 | ADR-0022 | Active-PR observation 2026-08-26: `tepp_topic_context_adapter` validates the full-data prerequisite outside `mlsirm-core` and fails with `CaseDeletionRefitEvidenceUnavailable`; the agreed successor requires exhaustive producer refits, independent anchor distributions, unique bijective alignment, and CPU/GPU receipts before domain-neutral Rust computes per-cell posterior prevalence contrasts | Proposed / producer contract pending |
