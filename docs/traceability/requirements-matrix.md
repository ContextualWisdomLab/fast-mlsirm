...
| Observed-score DIF screens | PRD-FR-021/043, fairness/invariance | ADR-0018 | `fast_mlsirm.delta_plot` (Angoff / Magis–Facon) plus MH/logistic/SIBTEST; method page `docs/delta_plot_dif.md` | Accepted screens; flags are review candidates, not fairness determinations |
...
| TEPP-anchored Event Lineage channel weights | PRD-FR-063, TRD-MLT-006 | ADR-0023 | Protected main admits continuous pair/channel evidence and exact accepted TEPP v1 identity in Rust, then returns `IndependentCriterionObservationsUnavailable`; no floor, Python fit, or local normalization. The v2 pair-posterior wire schema is published, but an accepted weight functional, complete producer artifact, Rust estimator, recovery evidence, and CPU/accelerator parity remain unavailable. | Proposed / fail-closed protected contract; end-to-end estimator unavailable |
...
| Finite-population proportion inference | PRD-FR-045/046, NFR-008 | ADR-0022, ADR-0025 | Rust sampling design and achieved SRSWOR estimator/variance/exact interval, PyO3/Python contracts, exhaustive coverage and published-table tests, doctoring evidence | Proposed |
