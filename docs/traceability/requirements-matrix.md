...
| TEPP posterior topic-context influence | PRD-FR-063, TRD-MLT-006..008 | ADR-0022 | `mlsirm_core::topic_context_influence` validates the full-data prerequisite and fails with `CaseDeletionRefitEvidenceUnavailable`; the agreed successor requires exhaustive producer refits, independent anchor distributions, unique bijective alignment, and CPU/GPU receipts before Rust computes per-cell posterior prevalence contrasts | Proposed / producer contract pending |
