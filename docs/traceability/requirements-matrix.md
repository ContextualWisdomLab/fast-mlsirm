...
| Observed-score DIF screens | PRD-FR-021/043, fairness/invariance | ADR-0015 | `fast_mlsirm.delta_plot` (Angoff / Magis–Facon) plus MH/logistic/SIBTEST; method page `docs/delta_plot_dif.md` | Accepted screens; flags are review candidates, not fairness determinations |
| Pairwise ranking (Bradley–Terry MM) | TRD-NUM-001 scoring/ranking | ADR-0016 | `fast_mlsirm.bradley_terry_mm`, `fast_mlsirm.bratt_mm`; method page `docs/bradley_terry_mm.md` | Accepted BT/Hunter MM and additive-ties BRATT only; Rao–Kupper/Davidson not implemented |
