...
- the document carries the required SPDX 2.3 creation envelope: exact SPDX/data-license/document identifiers, non-blank single-line document name, absolute fragment-free namespace URI with syntactically valid RFC 3986 percent escapes, exact `YYYY-MM-DDThh:mm:ssZ` UTC creation timestamp, and at least one non-blank single-line creator identity; missing, malformed, multiline, or unsupported values fail before attestation/publication;
