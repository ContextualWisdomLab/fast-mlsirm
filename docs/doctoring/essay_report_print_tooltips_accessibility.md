...
2. **Fallback Focus Visibility:** A previous attempt to remove the default focus outline on the `<main>` container using `main:focus { outline: none; }` (in favor of `main:focus-visible`) was reverted. Browsers that lack modern `:focus-visible` support would lose their keyboard focus indicator entirely on the skip-link target, causing an accessibility regression. We retain the explicit `main:focus-visible { outline: 3px solid Highlight; }` while leaving standard focus behavior intact for backward compatibility.
