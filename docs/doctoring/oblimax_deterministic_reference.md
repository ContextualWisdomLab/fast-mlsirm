...
`RotationCriterion::Oblimax` owns a deterministic CPU-binary64 arithmetic route for its criterion value and analytic gradient. For identical finite input bits, the implementation fixes the integer-power route to explicit multiplication and the logarithm route to package-owned IEEE-754 bit normalization plus a fixed 24-term Kahan-compensated `atanh` series. Before second/fourth moments are formed, one exact power-of-two multiplier derived from the maximum absolute loading conditions the represented range; the scale-free ratio `(sum4 / sum2) / sum2` is then formed before logarithm evaluation. The analytic gradient is factored through the fourth/second-moment ratio instead of subtracting two independently rounded reciprocal terms. When all represented nonzero loadings have the same magnitude, the gradient ratio uses that exact common represented square instead of reconstructing `sum4/sum2`, because the reduction quotient can round one ULP away with multiple equal supports. This preserves exact zero on the full equal-magnitude stationary manifold, not only the one-support special case. The repository therefore has a reviewable operator sequence and golden binary64 identity rather than delegating these operations to a platform transcendental implementation.
...
- exact zero objective and exact zero analytic gradient for positive and negative single-support finite fixtures;
- exact zero analytic gradient for multiple positive/negative supports with one represented nonzero magnitude, proving the whole equal-magnitude stationary manifold rather than one sparse instance;
...
Follow-up problem: the published Oblimax derivative written literally as `-(4 x^3 / sum4 - 4 x / sum2)` can leave a tiny nonzero binary64 residue when the two terms are mathematically identical. A single-support loading matrix has objective zero for every nonzero support magnitude and therefore an exact zero analytic gradient, yet the previous operator sequence produced a cancellation-sized residual for ordinary values such as `1.1` and `1.3`. Keeping the literal subtraction would preserve the earlier golden bits but would encode a false optimizer direction at an exact stationary point. The selected repair factors the same derivative through `sum4 / sum2`, making the stationary residual explicit while leaving the criterion, parameterization and gradient formula unchanged.

A second follow-up showed that factoring the derivative was necessary but not sufficient. With three equal-magnitude nonzero supports, e.g. `[1.3, -1.3, 1.3, 0, …]`, the represented common square is `1.6900000000000002`, while accumulated `sum4 / sum2` can round to adjacent `1.69`. The factor `1 - x^2 / (sum4 / sum2)` then becomes nonzero although the whole equal-magnitude manifold is mathematically stationary. Replacing the ratio algorithm globally would move ordinary-input golden bits and enlarge the causal surface. The selected repair therefore records whether all nonzero conditioned squares are bit-identical during the existing moment pass; only on that exact manifold does the gradient reuse the represented common square, while unequal supports retain the established `sum4 / sum2` route. This turns a representational identity into an explicit invariant without changing the public criterion or ordinary arithmetic route.
...
- Single-support stationary-gradient RED/GREEN: `720300d48c8e4b310f092780a2d805adb5bcd044` / `72e5e6a90c01a6a5aebd02b0dbb261a21ad8394c`, with golden-contract update `a689f7affeb5c9d545899ed835a273d7c42d10c0`.
- Equal-magnitude stationary-manifold RED: `5e78ac9dd6da8056dcf9f03ab34739897c8417c8`.
- Equal-magnitude stationary-manifold causal GREEN: `b00e91e437fbaf54f58c17a8c01bffd4341f5714`.
- Repair issue: #1747.
