...
For a posterior matrix of shape `(n_persons, n_nodes)`, the former expression materialized an element-wise product with the same shape before reduction. NumPy's current stable documentation defines `matmul`/`@` as matrix-product operations, including ordinary 2-D-by-1-D matrix-vector multiplication, and notes that optimized BLAS may be used when available. Exact performance depends on array shape, layout, linked numerical libraries, hardware, and runtime conditions, so this change makes no universal speedup claim.
...
NumPy Developers. (2026). *numpy.matmul—NumPy documentation*. https://numpy.org/doc/stable/reference/generated/numpy.matmul.html
