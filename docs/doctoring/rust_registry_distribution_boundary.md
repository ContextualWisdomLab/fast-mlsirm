...
The released product is `fast-mlsirm`, built with Maturin/PyO3 and published through the governed PyPI workflow. `crates/mlsirm-core` is the Rust numerical source of truth and `crates/fast-mlsirm-py` is the PyO3 binding crate behind that package. Neither implementation crate is a separately published crates.io product.
...
- `crates/fast-mlsirm-py/Cargo.toml` has `publish = false`;
...
A failure is a release-boundary defect. Do not repair it by deleting either guard merely because `CARGO_REGISTRY_TOKEN` is available. Credential availability is not product authority.

## If a standalone Rust crate is proposed later

Keep the relevant implementation crate at `publish = false` until an accepted successor ADR and implementation provide crate-name ownership verification, protected release identity, semantic-version/API compatibility policy, package/dry-run gates, Rust/rustdoc/Clippy/security/license/coverage/recovery evidence, SBOM/provenance, final-job-only registry credential exposure, published checksum verification and a clean downstream crates.io build.
