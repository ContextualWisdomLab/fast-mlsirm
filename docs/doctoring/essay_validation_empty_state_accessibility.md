...
WCAG 2.2 Success Criterion 4.1.3 requires status messages to be programmatically determinable through role or properties without receiving focus. W3C technique ARIA22 identifies `role="status"` as a sufficient technique for advisory status messages. The empty-state container therefore uses `role="status"` because the message is advisory status information and is not important enough to justify an alert.

WAI-ARIA defines `status` as a live-region role with implicit `aria-live="polite"` and `aria-atomic="true"`, and advises that a status should not receive focus as a consequence of a status change. We additionally serialize `aria-atomic="true"` explicitly so the intended whole-message atomicity remains obvious and interoperable to readers and tooling.
...
World Wide Web Consortium. (n.d.). *ARIA22: Using role=status to present status messages*. Web Accessibility Initiative. https://www.w3.org/WAI/WCAG22/Techniques/aria/ARIA22
