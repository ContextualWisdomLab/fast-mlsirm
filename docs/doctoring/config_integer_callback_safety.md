...
The hardened surface covers simulation sizes, latent dimension, and simulation `seed`, plus fit latent dimension, optimizer iteration/restart/history controls, quadrature node counts, marginal M-step count, latent-space integration point/seed controls, fit `seed`, and `verbose`. Numerical model ownership and Rust-first computation are unchanged.

## Test evidence

`tests/test_config_integer_callback_safety.py` provides hostile `__index__` regressions, valid-valued caller `int` subclasses, genuine NumPy scalar compatibility, write-back of narrow unsigned sizes, and a 2-by-256 simulation shape check that would fail if `n_items` wrapped. The original RED commit is `4c81e4dc465312d13f044b9b47e14d839af6cc1a`; exact-head hosted CI/security/package/coverage/review evidence remains authoritative as the branch advances.
