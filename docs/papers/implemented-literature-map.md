...
| Sinharay & Lu (2008), item parameters vs item-fit correlation, JEM | documented-only (the justification for S-X² over chi-square-G; already implemented) | `fitstats.rs::s_x2` |
...
| Ferrando, Lorenzo-Seva & Chico (2009), factor-analytic response-bias procedure, SEM | documented-only (needs keyed Likert content + SD markers absent from binary judge data) | group C spec |
