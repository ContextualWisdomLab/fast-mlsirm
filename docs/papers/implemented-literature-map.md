...
| Dueber (2021), `BifactorIndicesCalculator`; Rodriguez, Reise & Haviland (2016), methodological origin | implemented (continuous-indicator ECV-SS/SG/GS, item ECV, strict-pattern PUC, omega total, omega hierarchical, H, plus stable logistic-slope standardization; no hard-coded scoreability cutoff) | `bifactor_indices.rs`, `docs/bifactor_scoreability_indices.md`, `tests/bifactor_indices.rs`, `tests/bifactor_logit_indices.rs` |
