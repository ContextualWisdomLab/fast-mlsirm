...
- SCOPE: this is a 5-phase PR series, not one PR. The default-Bernoulli flag lets each phase land green independently, but bundling GPU+nominal+scoring+SE+Python into one review is unrealistic. Phase 4 (scoring/SE) is NOT optional for the VOB ordered-rubric deliverable -- Phase 1 fits GPCM but cannot score/report.
