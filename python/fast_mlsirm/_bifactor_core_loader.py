"""Load bifactor scoreability bindings from the installed Rust shared library.

The Rust extension exports both ``PyInit__core`` and
``PyInit__bifactor_core``. Python normally discovers one module per filename,
so this loader asks the standard extension loader to initialize the secondary
symbol from the already installed binary. It performs no source rewriting,
dynamic compilation, network access, or filesystem mutation.
"""
...
def _initialize_module() -> ModuleType:
    ...
    if spec is None:
        raise ImportError("could not create the fast_mlsirm bifactor extension spec")
