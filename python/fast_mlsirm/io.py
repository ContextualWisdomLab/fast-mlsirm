...
from .types import (DimensionalityDiagnostics, FitDiagnostics, FitResult,
                    MLSIRMParams, SimulationData)
...
def save_fit_diagnostics(diagnostics: FitDiagnostics, run_dir: str | Path) -> None:
    ...
    (out / "fit_diagnostics.json").write_text(
...
def save_dimensionality_diagnostics(
    ...
    (out / "dimension_diagnostics.json").write_text(
        json.dumps(payload, indent=2), encoding="utf-8"
    )
...
def load_factor_csv(path: str | Path) -> np.ndarray:
    ...
    with warnings.catch_warnings():
        ...
        return np.loadtxt(
            path, delimiter=",", skiprows=1, usecols=1, dtype=np.int64, ndmin=1
        )
...
def _write_factor_csv(path: Path, factor_id: np.ndarray) -> None:
    ...
    np.savetxt(
        path, data, delimiter=",", header="item_id,factor_id", comments="", fmt="%d"
    )
