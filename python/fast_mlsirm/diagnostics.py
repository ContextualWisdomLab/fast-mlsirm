...
def _distance_rmse(true_xi: np.ndarray, true_zeta: np.ndarray, est_xi: np.ndarray, est_zeta: np.ndarray) -> float:
    ...
    true_d_sq = true_xi_sq[:, None] - 2 * np.dot(true_xi, true_zeta.T) + true_zeta_sq[None, :]
    true_d = np.sqrt(np.maximum(true_d_sq, 0))

    est_xi_sq = (est_xi * est_xi).sum(axis=1)
    est_zeta_sq = (est_zeta * est_zeta).sum(axis=1)
    est_d_sq = est_xi_sq[:, None] - 2 * np.dot(est_xi, est_zeta.T) + est_zeta_sq[None, :]
    est_d = np.sqrt(np.maximum(est_d_sq, 0))
