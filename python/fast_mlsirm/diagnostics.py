...
from .types import (DimensionalityDiagnostics, FitDiagnostics, MLSIRMParams,
                    RecoveryReport)
...
def _distance_rmse(
    ...
    # Optimization: Use 2D dot products to avoid O(N*J*D) memory overhead
    ...
    est_d = np.sqrt(
        np.maximum(
            est_xi_sq[:, None] + est_zeta_sq[None, :] - 2 * np.dot(est_xi, est_zeta.T),
            0.0,
        )
    )
