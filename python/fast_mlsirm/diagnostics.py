...
def _distance_rmse(
    ...
    true_d_sq = np.maximum(
        true_xi_sq[:, None] + true_zeta_sq[None, :] - 2 * np.dot(true_xi, true_zeta.T),
        0.0,
    )
    true_d = np.sqrt(true_d_sq)

    est_xi_sq = np.einsum("ij,ij->i", est_xi, est_xi)
    est_zeta_sq = np.einsum("ij,ij->i", est_zeta, est_zeta)
    est_d_sq = np.maximum(
        est_xi_sq[:, None] + est_zeta_sq[None, :] - 2 * np.dot(est_xi, est_zeta.T), 0.0
    )
    est_d = np.sqrt(est_d_sq)
