...
def prepare_response(responses: np.ndarray, mask: np.ndarray | None = None) -> tuple[np.ndarray, np.ndarray]:
    ...
    # Optimized missing data checks: prefer .any() over .sum() to avoid integer casting overhead
...
def neg_loglik_and_grad(
    ...
    # Creates a one-hot encoding (J x D), projecting J items onto D dimensions
    indicator = np.zeros((e.shape[1], params.theta.shape[1]), dtype=e.dtype)
    indicator[np.arange(e.shape[1]), factors] = 1
    grad_theta = (e * a[None, :]) @ indicator
