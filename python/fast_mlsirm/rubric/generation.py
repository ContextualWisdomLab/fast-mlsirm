...
def _reject_duplicate_keys(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
    ...
    for key, value in pairs:
        if key in result:
            raise ValueError(
                f"contract_json contains a duplicate JSON object member: {key}"
            )
        result[key] = value
    return result
...
def _reject_nonfinite(_value: str) -> None:
    """Reject non-standard non-finite JSON constants."""
    raise ValueError("contract_json contains a non-finite JSON numeric value")
