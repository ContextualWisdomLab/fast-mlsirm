...
class TestletPilotDesign:
    """Content-addressed handoff for the Rust-backed binary testlet model.

    Each item's governed ``query_testlet_id`` becomes its testlet assignment.
    The wrapped binary design retains exact missingness, rater provenance, and
    replay-verified pilot identities. This artifact declares an estimation
    design only; it does not establish local dependence, adequate fit,
    convergence, fairness, scoreability, or validity.
    """
    ...
    def __post_init__(self, _design_token: object | None) -> None:
        ...
        if _design_token is not _TESTLET_DESIGN_TOKEN:
            raise ValueError(
                "TestletPilotDesign must be created by "
    ...
    @property
    def testlet_ids(self) -> tuple[str, ...]:
        """Return descriptive testlet identifiers in numeric-id order."""
    ...
    @property
    def testlet_item_counts(self) -> tuple[int, ...]:
        """Return the number of items assigned to each declared testlet."""
        return tuple(
            self.item_testlet_ids.count(testlet_index)
            for testlet_index in range(len(self.testlet_ids))
        )

    @property
    def multi_item_testlet_ids(self) -> tuple[str, ...]:
        """Return testlets whose local-dependence variance can be estimated."""
        return tuple(
            testlet_id
            for testlet_id, item_count in zip(
                self.testlet_ids,
                self.testlet_item_counts,
                strict=True,
            )
            if item_count > 1
        )
    ...
    @property
    def singleton_testlet_ids(self) -> tuple[str, ...]:
        """Return singleton testlets whose variance is structurally fixed to zero."""
        return tuple(
            testlet_id
            for testlet_id, item_count in zip(
                self.testlet_ids,
    ...
    def _content_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "testlet_ids": list(self.testlet_ids),
            "item_testlet_ids": list(self.item_testlet_ids),
            "testlet_item_counts": list(self.testlet_item_counts),
            "multi_item_testlet_ids": list(self.multi_item_testlet_ids),
            "singleton_testlet_ids": list(self.singleton_testlet_ids),
    ...
    def testlet_id_array(self) -> np.ndarray:
        """Return a fresh per-item zero-based testlet assignment vector."""
        return self.binary_design.factor_id_array()

    def to_fit_testlet_kwargs(self) -> dict[str, Any]:
        """Return copied arguments accepted directly by ``fast_mlsirm.fit_testlet``.

        Model-family and numerical options remain explicit caller decisions and
        are validated by ``fit_testlet``. The handoff supplies only the governed
        response matrix and descriptive-to-numeric testlet mapping, avoiding an
        unsupported claim that one Rasch or 2PL specification is universally
        appropriate.
        """
        return {
...
def build_testlet_pilot_design(
    ...
    allocation validation. Its sorted ``query_testlet_id`` mapping is reused
    unchanged as the testlet assignment consumed by the Rust-backed fitter.
    """
