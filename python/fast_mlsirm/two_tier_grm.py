...
def fit_two_tier_grm(
    ...
    ``q_primary``/``q_specific`` are required Gauss-Hermite node counts (one
    of ``(7, 11, 15, 21, 31, 41)`` — the embedded rules that exist, hence
    the only accepted values); no default is offered, because no accuracy
    target is on file to source one against (Project rule, issue #1929).
    Study settings use >= 121 nodes per dimension once the shared gate
    supports them (chosen by precision convergence, e.g. 121 vs 241).
    ...
    if q_primary_int not in _SUPPORTED_Q:
        raise ValueError(f"q_primary must be one of {_SUPPORTED_Q}")
