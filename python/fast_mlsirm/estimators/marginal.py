...
def fit_gpcm_numpy(y, n_cat, q_theta=21, max_iter=80, tol=1e-6):
    ...
    # Optimized: precompute a one-hot category mask outside the iterative loop to eliminate intermediate allocations
    y_mask = (y[:, :, None] == scores).astype(np.float64)

    for it in range(1, max_iter + 1):
        for i in range(n_items):
            # Optimized: use matrix multiplication with precomputed mask to eliminate intermediate allocations and leverage BLAS
            r = post.T @ y_mask[:, i, :]
