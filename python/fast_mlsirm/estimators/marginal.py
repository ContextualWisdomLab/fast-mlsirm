...
def _person_logliks(
    ...
    Returns (log_lt, log_zdx, log_lp): log_lt has shape (P, D, Qt, Nx); log_zdx (P, D, Nx);
    log_lp (P,).
    """
    delta = logp1 - logp0  # (S, I, Qt, Nx)
    pos = np.where(observed, y, 0.0)  # (P, I)
    log_lt = c0[s_of_person]  # (P, D, Qt, Nx) — copy via fancy indexing
    ...
    lw = t_logw[None, None, :, None] + log_lt  # (P, D, Qt, Nx)
...
def _posteriors(
    ...
    pt = np.exp(t_logw[None, None, :, None] + log_lt - log_zdx[:, :, None, :])
...
def fit_marginal_numpy(
    ...
    for _iteration in range(max_iter):
        ...
        if kind in {"single", "singlefree", "multigroup"}:
            ...
            log_lt, log_zdx, log_lp = _person_logliks(
                y,
                observed,
                factor_id,
                logp1,
            ...
            post = _posteriors(log_lt, log_zdx, log_lp, t_logw, x_logw)
            _accumulate(
                post,
                w_irt,
                y,
