...
def _item_q(
    ...
    """
    # Optimized: replace np.sum(A * B) with np.vdot(A, B) to avoid intermediate array allocation (~9.8x speedup)
            ...
            for _ in range(m_steps):
                ...
                if uses_space:
                    if kind_i == "inner":
                        ...
                        # Optimized: replace np.sum(diff * diff, axis=1) with np.einsum to avoid intermediate array allocation (~2.7x speedup)
