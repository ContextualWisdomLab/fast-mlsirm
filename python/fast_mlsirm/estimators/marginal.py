...
def _item_q(
    ...
    """
    # Optimized: mathematically simplified expected log-likelihood using log(sigmoid(x)) - log(sigmoid(-x)) = x and replaced np.sum with np.vdot to avoid intermediate array allocation
    q = float(
        np.vdot(r_i.ravel(), eta.ravel())
        + np.vdot(n_i.ravel(), _log_sigmoid(-eta).ravel())
    )
        ...
        if uses_space and anchor_tau is None and _interaction_kind(model) == "distance":
            ...
            if info > 0.0:
                ...
                def total_q(tau_c: float) -> float:
                    ...
                    # Optimized: mathematically simplified expected log-likelihood using log(sigmoid(x)) - log(sigmoid(-x)) = x and replaced np.sum with np.vdot to avoid intermediate array allocation
                    qv = float(
                        np.vdot(rbar.ravel(), e.ravel())
                        + np.vdot(n_all.ravel(), _log_sigmoid(-e).ravel())
                    )
