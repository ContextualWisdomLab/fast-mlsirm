...
def _person_logliks(
    ...
    for d in range(n_dims):
        ...
        # Optimized: Replace slow einsum gathering with masked BLAS @ per context subset
        for s in range(delta_d.shape[0]):
            sel = s_of_person == s
            if not sel.any():
                continue

            # (P_s, I_d) @ (I_d, Qt * Nx) -> (P_s, Qt * Nx) -> (P_s, Qt, Nx)
            qt_nx = delta_d.shape[2] * delta_d.shape[3]
            l[sel, d] += (pos_d[sel] @ delta_d[s].reshape(-1, qt_nx)).reshape(
                -1, delta_d.shape[2], delta_d.shape[3]
            )
            if miss_d.any():
                l[sel, d] -= (miss_d[sel] @ logp0_d[s].reshape(-1, qt_nx)).reshape(
                    -1, delta_d.shape[2], delta_d.shape[3]
                )
...
def _accumulate(
    ...
    for s in range(n_ctx):
        ...
        pos = np.where(observed[sel], y[sel], 0.0)  # (P_s, I)
        miss = (~observed[sel]).astype(np.float64)  # (P_s, I)

        # Optimized: Replace 4D einsum accumulation with masked BLAS @ per dimension
        qt_nx = wpost.shape[2] * wpost.shape[3]
        for d in range(wpost.shape[1]):
            items_d = np.flatnonzero(factor_id == d)
            if len(items_d) == 0:
                continue

            wpost_sel_d_flat = wpost[sel, d].reshape(-1, qt_nx)  # (P_s, Qt * Nx)
            rbar[s, items_d] += (pos[:, items_d].T @ wpost_sel_d_flat).reshape(
                -1, wpost.shape[2], wpost.shape[3]
            )
            if miss.any():
                mbar[s, items_d] += (miss[:, items_d].T @ wpost_sel_d_flat).reshape(
                    -1, wpost.shape[2], wpost.shape[3]
                )
