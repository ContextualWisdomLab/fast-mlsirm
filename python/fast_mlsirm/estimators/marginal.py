...
def _preflight_xi_node_count(
    *,
    ...
    if q_xi not in SUPPORTED_Q:
        raise ValueError(
            f"unsupported quadrature size {q_xi}; supported: {list(SUPPORTED_Q)}"
        )
    if latent_dim < 1:
...
def _xi_grid(q_xi: int, latent_dim: int) -> tuple[np.ndarray, np.ndarray]:
    ...
    n_points = _bounded_tensor_node_count(q_xi, latent_dim, limit=_MAX_TENSOR_XI_NODES)
...
def fit_marginal_numpy(
    ...
    if (
        n_persons * max(n_items, n_dims) * int(q_theta) * int(n_xi)
        > MAX_MARGINAL_WORKING_SET
    ):
        raise ValueError(
        ...
        for i in range(n_items):
            ...
            def eta_of(alpha_c: float, b_c: float, zeta_c: np.ndarray) -> np.ndarray:
                ...
                if kind_i == "distance":
                    ...
                    zeta_row = np.ascontiguousarray(
                        zeta_c.reshape(1, -1), dtype=np.float64
                    )
                    x_c = np.ascontiguousarray(x_grid, dtype=np.float64)
                    dist = _pairwise_euclidean_distances(
                        zeta_row,
...
def _gpcm_m_step_item(params0, theta_nodes, r_counts, n_newton=10):
    ...
    for _ in range(n_newton):
        ...
        ):  # pragma: no cover - ridge-regularized finite Hessian is non-singular
            step = g
        p = p - step
...
def fit_gpcm_numpy(y, n_cat, q_theta=21, max_iter=80, tol=1e-6):
    ...
    # We remove the array-allocation in E-step instead of a hard limit because:
    # "To resolve 'Denial of Service via Excessive Memory Allocation' vulnerabilities in scientific computing tasks (like MMLE estimators), do not introduce arbitrary hardcoded input size limits (e.g., `y.size > 10_000_000`), as this creates breaking regressions for legitimate large datasets. Instead, fix the underlying inefficiency by optimizing intermediate array allocations, such as replacing broadcasted multiplications with matrix multiplications."
    ...
    def estep(current_params):
        ...
        item_lp = np.array(item_lp_list)
        log_node = np.zeros((n_persons, q_theta))
        # Optimized: precompute one-hot mask and use matrix multiplication to eliminate large intermediate array allocations
        for k in range(k_cat):
            log_node += (y == k).astype(np.float64) @ item_lp[:, :, k]
    ...
    # Optimized: precompute one-hot mask and use matrix multiplication to eliminate intermediate boolean array allocations in the M-step loop
    ...
    for it in range(1, max_iter + 1):
        ...
        if not np.isfinite(
            next_ll
        ):  # pragma: no cover - stable log-sum-exp keeps the likelihood finite
