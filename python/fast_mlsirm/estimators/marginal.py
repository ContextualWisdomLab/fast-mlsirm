...
def fit_gpcm_numpy(y, n_cat, q_theta=21, max_iter=80, tol=1e-6):
    ...
    for it in range(1, max_iter + 1):
        for i in range(n_items):
            # Optimized boolean masking and sum: avoid looping over categorical factors by using a dense 2D numeric mask and matrix multiplication
            r = post.T @ (y[:, i, None] == np.arange(k_cat)).astype(post.dtype)
