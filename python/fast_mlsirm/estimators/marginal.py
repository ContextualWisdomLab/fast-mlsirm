...
def _accumulate(
    ...
    P_total = wpost.shape[0]
    ones_P = np.ones(P_total)

    for s in range(n_ctx):
        sel = s_of_person == s
        Ps = np.count_nonzero(sel)
        if Ps == 0:
            continue

        wps = wpost[sel]

        # Optimize .sum(axis=0) with BLAS reduction
        wps_flat = wps.reshape(Ps, -1)
        nbar[s] += (ones_P[:Ps] @ wps_flat).reshape(wps.shape[1:])

        pos = np.where(observed[sel], y[sel], 0.0)  # (Ps, I)
        miss = (~observed[sel]).astype(np.float64)

        # Optimize einsum over large axes with BLAS matrix multiplication
        for d in range(wps.shape[1]):
            items_d = np.where(factor_id == d)[0]
            if len(items_d) == 0:
                continue

            # (Ps, Qt * Nx)
            wps_d = wps[:, d, :, :].reshape(Ps, -1)

            # (len(items_d), Ps) @ (Ps, Qt * Nx) -> (len(items_d), Qt * Nx)
            r_d = pos[:, items_d].T @ wps_d
            rbar[s, items_d] += r_d.reshape(len(items_d), wps.shape[2], wps.shape[3])

            if miss.any():
                m_d = miss[:, items_d].T @ wps_d
                mbar[s, items_d] += m_d.reshape(
                    len(items_d), wps.shape[2], wps.shape[3]
                )
