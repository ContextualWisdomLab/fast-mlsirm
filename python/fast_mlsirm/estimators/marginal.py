...
def _person_logliks(
    ...
    Returns (l_out, log_zdx, log_lp): l_out has shape (P, D, Qt, Nx); log_zdx (P, D, Nx);
    log_lp (P,).
    """
    delta = logp1 - logp0  # (S, I, Qt, Nx)
    pos = np.where(observed, y, 0.0)  # (P, I)
    l_out = c0[s_of_person]  # (P, D, Qt, Nx) — copy via fancy indexing
    ...
    for d in range(n_dims):
        ...
        l_out[:, d] += np.einsum("pi,piqx->pqx", pos_d, delta_d[s_of_person], optimize=True)
        if miss_d.any():
            l_out[:, d] -= np.einsum(
                "pi,piqx->pqx", miss_d, logp0_d[s_of_person], optimize=True
            )
    lw = t_logw[None, None, :, None] + l_out  # (P, D, Qt, Nx)
...
def _posteriors(
    ...
    pt = np.exp(t_logw[None, None, :, None] + l_out - log_zdx[:, :, None, :])
...
def fit_marginal_numpy(
    ...
    for _iteration in range(max_iter):
        ...
        if kind in {"single", "singlefree", "multigroup"}:
            ...
            l_out, log_zdx, log_lp = _person_logliks(
                y,
                observed,
                factor_id,
                logp1,
            ...
            post = _posteriors(l_out, log_zdx, log_lp, t_logw, x_logw)
            _accumulate(
                post,
                w_irt,
                y,
        ...
        if uses_space and anchor_tau is None and _interaction_kind(model) == "distance":
            ...
            if info > 0.0:
                ...
                def total_q(tau_c: float) -> float:
                    ...
                    qv = float(np.sum(rbar * e + n_all * _log_sigmoid(-e)))
        ...
        if w_cov is not None:
            ...
            if info_d > 0.0:
                ...
                def q_of_delta(delta_c: float) -> float:
                    ...
                    e = eta_delta(delta_c)
                    return float(np.sum(rbar * e + n_all * _log_sigmoid(-e)))
