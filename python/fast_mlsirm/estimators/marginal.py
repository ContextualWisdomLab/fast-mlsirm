...
def _person_logliks(
    ...
    Returns (ll, log_zdx, log_lp): ll has shape (P, D, Qt, Nx); log_zdx (P, D, Nx);
    log_lp (P,).
    """
    delta = logp1 - logp0  # (S, I, Qt, Nx)
    pos = np.where(observed, y, 0.0)  # (P, I)
    ll = c0[s_of_person]  # (P, D, Qt, Nx) — copy via fancy indexing
    ...
    lw = t_logw[None, None, :, None] + ll  # (P, D, Qt, Nx)
...
def _posteriors(
    ...
    pt = np.exp(t_logw[None, None, :, None] + ll - log_zdx[:, :, None, :])
...
def fit_marginal_numpy(
    ...
    for _iteration in range(max_iter):
        ...
        if kind in {"single", "singlefree", "multigroup"}:
            ...
            ll, log_zdx, log_lp = _person_logliks(
                y,
                observed,
                factor_id,
                logp1,
            ...
            post = _posteriors(ll, log_zdx, log_lp, t_logw, x_logw)
            _accumulate(
                post,
                w_irt,
                y,
