...
def _item_q(
    ...
    """
    # Optimized: r_i * _log_sigmoid(eta) + (n_i - r_i) * _log_sigmoid(-eta)
    # mathematically simplifies to r_i * eta + n_i * _log_sigmoid(-eta)
        ...
        if uses_space and anchor_tau is None and _interaction_kind(model) == "distance":
            ...
            if info > 0.0:
                ...
                def total_q(tau_c: float) -> float:
                    ...
                    # Optimized: mathematically simplifies, avoids temporary allocation
                    qv = float(
                        np.vdot(rbar, e) + np.vdot(n_all, _log_sigmoid(-e))
                    )
