...
def _build_tables(
    ...
    if kind == "distance":
        # Optimized 3D distance computation: replaced O(I*Nx*K) 3D broadcast with O(I*Nx) 2D einsum and dot product
        x_sq = np.einsum("nk,nk->n", x_grid, x_grid)
        z_sq = np.einsum("ik,ik->i", zeta, zeta)
        dist_sq = x_sq[None, :] + z_sq[:, None] - 2 * np.dot(zeta, x_grid.T)
...
def _person_logliks(
    ...
    Returns (l_val, log_zdx, log_lp): l_val has shape (P, D, Qt, Nx); log_zdx (P, D, Nx);
    log_lp (P,).
    """
...
def fit_marginal_numpy(
    ...
    for _iteration in range(max_iter):
            ...
            for _ in range(m_steps):
                ...
                if uses_space:
                    if kind_i == "inner":
                        ...
                        # Bypass the intermediate 2D array allocation when computing distances for gradients
                        dist = np.sqrt(eps_distance + np.einsum("nk,nk->n", diff, diff))
        ...
        if uses_space and anchor_tau is None and _interaction_kind(model) == "distance":
            gamma = float(np.exp(tau))
            # Optimized distance computation: replace O(I*Nx*K) 3D broadcast with O(I*Nx) 2D dot product
            x_sq = np.einsum("nk,nk->n", x_grid, x_grid)
            z_sq = np.einsum("ik,ik->i", zeta, zeta)
            dist_sq = x_sq[None, :] + z_sq[:, None] - 2 * np.dot(zeta, x_grid.T)
            dist = np.sqrt(eps_distance + np.maximum(dist_sq, 0.0))  # (I, Nx)
        ...
        if w_cov is not None:
            ...
            kind_i = _interaction_kind(model)
            if kind_i == "distance":
                # Optimized 3D distance computation: replace O(I*Nx*K) 3D broadcast with O(I*Nx) 2D dot product
                x_sq = np.einsum("nk,nk->n", x_grid, x_grid)
                z_sq = np.einsum("ik,ik->i", zeta, zeta)
                dist_sq = x_sq[None, :] + z_sq[:, None] - 2 * np.dot(zeta, x_grid.T)
