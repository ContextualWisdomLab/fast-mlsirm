...
def fit_marginal_numpy(
    ...
    for _iteration in range(max_iter):
        ...
        if w_cov is not None:
            ...
            kind_i = _interaction_kind(model)
            if kind_i == "distance":
                # Optimized distance computation: replace O(I*Nx*D) 3D broadcast with O(I*Nx) 2D dot product
                x_sq = np.einsum("ij,ij->i", x_grid, x_grid)
                z_sq = np.einsum("ij,ij->i", zeta, zeta)
                dist_sq = z_sq[:, None] + x_sq[None, :] - 2 * np.dot(zeta, x_grid.T)
            ...
            # Reduce over (Qt, Nx) first to avoid broadcasting w_cov into a massive (S, I, Qt, Nx) array
            resid_sum = resid.sum(axis=(2, 3))
            grad_d = float(np.vdot(resid_sum, w_cov))

            info_sum = (n_all * prob * (1.0 - prob)).sum(axis=(2, 3))
            info_d = float(np.vdot(info_sum, w_cov * w_cov))
