...
def fit_gpcm_numpy(y, n_cat, q_theta=21, max_iter=80, tol=1e-6):
    ...
    for it in range(1, max_iter + 1):
        for i in range(n_items):
            # Optimized: avoid allocating intermediate boolean arrays and unvectorized row-wise summations in NumPy categorical reductions using an aggregated matrix multiplication.
            r = np.stack([(y[:, i] == k).astype(post.dtype, copy=False).T @ post for k in range(k_cat)], axis=1)
