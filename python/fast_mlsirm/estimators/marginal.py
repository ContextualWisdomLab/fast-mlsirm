...
def _person_logliks(
    ...
    Returns (log_lik_dtx, log_zdx, log_lp): log_lik_dtx has shape (P, D, Qt, Nx); log_zdx (P, D, Nx);
    log_lp (P,).
    """
    delta = logp1 - logp0  # (S, I, Qt, Nx)
    pos = np.where(observed, y, 0.0)  # (P, I)
    log_lik_dtx = c0[s_of_person]  # (P, D, Qt, Nx) — copy via fancy indexing
    ...
    lw = t_logw[None, None, :, None] + log_lik_dtx  # (P, D, Qt, Nx)
    ...
    return log_lik_dtx, log_zdx, log_lp


def _posteriors(
    log_lik_dtx: np.ndarray,
    log_zdx: np.ndarray,
    log_lp: np.ndarray,
    t_logw: np.ndarray,
    x_logw: np.ndarray,
    ...
    pt = np.exp(t_logw[None, None, :, None] + log_lik_dtx - log_zdx[:, :, None, :])
...
def fit_marginal_numpy(
    ...
    for _iteration in range(max_iter):
        ...
        if kind in {"single", "singlefree", "multigroup"}:
            ...
            log_lik_dtx, log_zdx, log_lp = _person_logliks(
                y,
                observed,
                factor_id,
                logp1,
            ...
            post = _posteriors(log_lik_dtx, log_zdx, log_lp, t_logw, x_logw)
            _accumulate(
                post,
                w_irt,
                y,
