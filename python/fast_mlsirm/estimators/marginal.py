...
def _build_tables(
    ...
    if kind == "distance":
        # Optimized Euclidean distance computation to avoid massive O(I * Nx * K) intermediate array allocation.
        # Uses algebraic expansion (x^2 + z^2 - 2xz) with `np.einsum` for fast O(K) reduction and `np.dot` for pairwise interactions.
    ...
    for _iteration in range(max_iter):
        ...
        for i in range(n_items):
            ...
            def eta_of(alpha_c: float, b_c: float, zeta_c: np.ndarray) -> np.ndarray:
                ...
                if kind_i == "distance":
                    ...
                    # Optimized distance calculations: avoided (diff * diff) intermediate O(Nx * K) array allocation by using `np.einsum`
        ...
        if w_cov is not None:
            ...
            if kind_i == "distance":
                ...
                dist_sq = x_sq[None, :] + z_sq[:, None] - 2 * np.dot(zeta, x_grid.T)
                dist_sq = np.maximum(dist_sq, 0.0)
                distz = np.sqrt(eps_distance + dist_sq)  # (I, Nx)
