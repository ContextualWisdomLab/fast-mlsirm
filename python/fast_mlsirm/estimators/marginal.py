...
def _person_logliks(
    ...
    Returns (loglik_cond, log_zdx, log_lp): loglik_cond has shape (P, D, Qt, Nx); log_zdx (P, D, Nx);
    log_lp (P,).
    """
    delta = logp1 - logp0  # (S, I, Qt, Nx)
    pos = np.where(observed, y, 0.0)  # (P, I)
    loglik_cond = c0[s_of_person]  # (P, D, Qt, Nx) — copy via fancy indexing
    ...
    lw = t_logw[None, None, :, None] + loglik_cond  # (P, D, Qt, Nx)
    ...
    return loglik_cond, log_zdx, log_lp


def _posteriors(
    loglik_cond: np.ndarray,
    log_zdx: np.ndarray,
    log_lp: np.ndarray,
    t_logw: np.ndarray,
    x_logw: np.ndarray,
    ...
    pt = np.exp(t_logw[None, None, :, None] + loglik_cond - log_zdx[:, :, None, :])
...
def fit_marginal_numpy(
    ...
    for _iteration in range(max_iter):
        ...
        if kind in {"single", "singlefree", "multigroup"}:
            ...
            loglik_cond, log_zdx, log_lp = _person_logliks(
                y,
                observed,
                factor_id,
                logp1,
            ...
            post = _posteriors(loglik_cond, log_zdx, log_lp, t_logw, x_logw)
            _accumulate(
                post,
                w_irt,
                y,
