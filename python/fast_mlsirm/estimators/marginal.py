...
def fit_marginal_numpy(
    ...
    def eap_accumulate(s_all: np.ndarray, w_outer: np.ndarray) -> None:
        ...
        post = _posteriors(l, log_zdx, log_lp, t_logw, x_logw)
        px = (
            np.einsum("pdqx,p->px", post, w_outer) / n_dims
        )  # (P, Nx) — same for every d
        xi_eap[:] += px @ x_grid
        theta_s = (
            ctx["shift"][s_all][:, :, None] + ctx["scale"][s_all][:, :, None] * t_nodes
        )
        theta_eap[:] += np.einsum(
            "pdtx,p,pdt->pd", post, w_outer, theta_s, optimize=True
        )
        theta_m2[:] += np.einsum(
            "pdtx,p,pdt->pd", post, w_outer, theta_s**2, optimize=True
        )
...
def score_eap(
    ...
    post = _posteriors(l, log_zdx, log_lp, t_logw, x_logw)
    px = np.einsum("pdqx->px", post) / n_dims  # (P, Nx)
    xi_eap = px @ x_grid
