...
def _item_q(
    ...
    """
    # Optimized: r_i * x + n_i * _log_sigmoid(-x) is equivalent to r_i * _log_sigmoid(x) + (n_i - r_i) * _log_sigmoid(-x)
    # since _log_sigmoid(x) - _log_sigmoid(-x) = x. This avoids one call to expensive _log_sigmoid.
        ...
        if uses_space and anchor_tau is None and _interaction_kind(model) == "distance":
            ...
            if info > 0.0:
                ...
                def total_q(tau_c: float) -> float:
                    ...
                    qv = float(
                        # Optimized: mathematically simplified to avoid double evaluation of _log_sigmoid
