...
def infit_outfit(
    ...
    obs_cast = observed.astype(np.float64, copy=False)
    outfit = np.einsum("ij,ij->j", resid2 / v, obs_cast) / n_obs
    infit = resid2.sum(axis=0) / np.maximum(np.einsum("ij,ij->j", v, obs_cast), 1e-12)
