...
def _icc_grid(
    ...
    if uses_space:
        ...
        z_sq = np.einsum("ik,ik->i", params.zeta, params.zeta)
        dist_sq = x_sq[None, :] + z_sq[:, None] - 2 * np.dot(params.zeta, x_grid.T)
...
def person_fit(
    ...
    if uses_space:
        # Optimized 3D distance computation: replaced O(N*J*D) 3D broadcast with O(N*J) 2D dot product
        xi = np.asarray(params.xi)
        zeta = np.asarray(params.zeta)
        xi_sq = np.einsum("nk,nk->n", xi, xi)
        zeta_sq = np.einsum("ik,ik->i", zeta, zeta)
