...
def infit_outfit(
    ...
    v = p * (1.0 - p)
    # Optimized calculation: replace intermediate array allocation and sum(axis=0) with einsum
    obs_f = observed.astype(np.float64)
    n_obs = np.maximum(observed.sum(axis=0), 1)
    resid2_unmasked = (y - p) ** 2
    outfit = np.einsum("ij,ij->j", resid2_unmasked / v, obs_f) / n_obs
    infit = np.einsum("ij,ij->j", resid2_unmasked, obs_f) / np.maximum(
        np.einsum("ij,ij->j", v, obs_f), 1e-12
    )
