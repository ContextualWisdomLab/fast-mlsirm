...
def _m2_numpy(
    ...
    # ⚡ Bolt: Vectorize pairwise proportion calculations
    # Use matrix multiplication (@) to compute pair co-occurrence means across columns
    # instead of an O(N^2) explicit Python loop doing elementwise masking.
    Y_bool = (yc != 0.0).astype(float)
    p_obs[:n_items] = Y_bool.mean(axis=0)
    pair_means = (Y_bool.T @ Y_bool) / n_c
    i_idx, j_idx = np.triu_indices(n_items, k=1)
    p_obs[n_items:] = pair_means[i_idx, j_idx]
