...
## 2026-07-29 - [DoS via Unbounded JSON Loading in Diagnostic Report Rendering]
**Vulnerability:** `json.loads(source.read_text(encoding="utf-8"))` was used to read a potentially large file into memory all at once in `python/fast_mlsirm/report.py`. This exposes the application to Denial of Service (DoS) attacks via memory exhaustion if a malicious or excessively large file is provided.
**Learning:** Using `read_text()` followed by `json.loads()` on unchecked files is dangerous. It reads the entire content into memory at once and doesn't limit the JSON nesting depth, allowing DoS vectors via either pure memory exhaustion or recursion limits.
**Prevention:** Always use bounded loaders, such as the pre-existing `_load_json_bounded` utility in `io.py`, which enforces memory limits and structural constraints like maximum nesting depth when parsing untrusted or user-supplied JSON files.
