...
## 2026-08-19 - [DoS via Unbounded subprocess JSON Decoding]
**Vulnerability:** External command output via `subprocess` was deserialized using unbounded `json.loads(completed.stdout)` in several governance and due-diligence scripts. Large outputs from GitHub CLI (`gh`) could exhaust memory or trigger recursion limits if deeply nested.
**Learning:** Never trust the output of an external command to be inherently bounded when decoding into objects. Even if the output comes from a known CLI tool, it should be processed safely, using the repository's native `parse_json_bounded` utility for defense-in-depth, preventing DoS.
**Prevention:** Always use `parse_json_bounded` or equivalent safe parsers instead of raw `json.loads` when deserializing data from `subprocess.stdout`.
