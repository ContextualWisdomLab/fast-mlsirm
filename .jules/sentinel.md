...
## 2026-10-18 - [CSP Unsafe-Inline Vulnerability in HTML Reports]
**Vulnerability:** The HTML reports generated by `fast_mlsirm` (`python/fast_mlsirm/report.py` and `python/fast_mlsirm/scoring/essay/report_html.py`) used `style-src 'unsafe-inline'` in their Content Security Policy (CSP). This allows any injected inline styles to execute, opening the door for CSS injection attacks, which can be leveraged to exfiltrate data or deface the page in scenarios where attacker-controlled input might bypass escaping logic.
**Learning:** Using `unsafe-inline` for CSS is a security risk. A more robust approach for static inline styles in standalone HTML reports is to compute the SHA-256 hash of the CSS content and embed that specific hash in the CSP's `style-src` directive.
**Prevention:** Instead of `unsafe-inline`, dynamically compute the base64-encoded SHA-256 hash of the intended inline CSS (e.g. `hashlib.sha256(css_content.encode('utf-8'))`) and add it to the `style-src` directive (e.g. `style-src 'sha256-...'`).

## 2026-10-18 - [CSP style-src hashes do not cover style attributes]
**Vulnerability:** When migrating from `style-src 'unsafe-inline'` to a hash-based CSP (`style-src 'sha256-...'`) for inline `<style>` blocks, the hash only permits the `<style>` block itself. It does NOT permit inline `style="..."` attributes on individual HTML elements (such as dynamically sized chart bars `style="width: 37.5%"`). This causes browsers to block the inline attributes, silently breaking visual elements that rely on them.
**Learning:** CSP3 hashes apply to elements, not attributes. If a document requires both a strictly hashed `<style>` element and inline `style="..."` attributes on elements, they cannot share the `style-src` directive without re-introducing `'unsafe-inline'` which overrides the hash.
**Prevention:** Use the `style-src-elem` directive for the hashed `<style>` block (e.g., `style-src-elem 'sha256-...'`), and use the `style-src` directive for the inline attributes (e.g., `style-src 'unsafe-inline'`). Modern browsers will use `style-src-elem` for elements and `style-src` for attributes, maintaining security for the block while allowing the required inline styles.
