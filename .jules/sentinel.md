...
## 2026-08-25 - [CSP unsafe-inline Risk & Mitigation]
**Vulnerability:** HTML reports generated by `fast_mlsirm` used `'unsafe-inline'` in their Content-Security-Policy (CSP) `style-src` directives. This inherently weakens CSP effectiveness, creating a risk where an attacker could theoretically inject malicious styles to execute a CSS-based attack (like data exfiltration via background images) if they somehow bypassed input validation.
**Learning:** While the reports appear to only contain hardcoded and tightly controlled structural output, adhering strictly to defense-in-depth principles means avoiding `'unsafe-inline'`. Python's `hashlib.sha256` can be combined with `base64` to generate the exact required digest of inline CSS. The hashed string must precisely match the raw text content inside the `<style>` tag.
**Prevention:** Never use `'unsafe-inline'` in CSP for generated HTML artifacts. Instead, dynamically compute the SHA-256 base64 digest of the raw CSS string (e.g. `hashlib.sha256(css.encode('utf-8'))`), avoiding formatting inconsistencies, and include it explicitly via `style-src 'sha256-<hash>'`.
