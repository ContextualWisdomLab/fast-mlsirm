...
**Vulnerability:** JSON 깊이(depth) 검증에서 닫는 괄호(`]` 또는 `}`)를 만날 때 조건 없이 `depth -= 1`을 수행하면 잘못된 입력에서 깊이 계산이 부정확해질 수 있습니다. 그러나 앞에 짝이 맞지 않는 닫는 괄호가 있는 입력은 유효하지 않은 JSON이므로, 그 자체로 깊이 제한 우회나 재귀 기반 DoS를 일으킨다고 설명해서는 안 됩니다.
