...
## 2026-08-19 - [CWE-617: Avoid Using Asserts for Validation In Production Code]
**Vulnerability:** `assert` statements were used in `python/fast_mlsirm/llm_judge.py` and `python/fast_mlsirm/judge_calibration.py` for data validation and invariant checks. Assertions are entirely stripped out when Python is executed with the `-O` or `-OO` optimization flag, completely bypassing these safety validations.
**Learning:** Relying on assertions for essential validation allows for security boundaries to be bypassed unintentionally in production environments where optimization flags are common. This is categorized under CWE-617.
**Prevention:** Never use `assert` for critical data validation, bounds checking, or security constraints. Always use explicit `if` conditions that raise standard exceptions like `ValueError` or `RuntimeError`.
