...
## 2026-08-16 - [Subprocess Timeout Exploitation Risk]
**Vulnerability:** External system or API commands executed via `subprocess.run()` without an explicit timeout argument can hang indefinitely if the network drops or the external service becomes unresponsive. This can consume process slots and block CI runners forever, constituting a local Denial of Service (DoS).
**Learning:** Adding a `timeout` prevents unbounded waiting. However, simply adding `timeout` raises `subprocess.TimeoutExpired`. In robust systems like polling retry loops, this exception must be caught explicitly to properly format an error rather than ungracefully crashing the calling script.
**Prevention:** Always add a `timeout` value to `subprocess.run` calls, and catch `subprocess.TimeoutExpired` to handle the failure gracefully.
## 2026-08-16 - [Command Injection via Python Executable Argument]
**Vulnerability:** Scripts accepting a `--python` argument used it directly in `subprocess.run` calls without validating the path. This allowed arbitrary executable invocation by attackers manipulating this argument.
**Learning:** External arguments passed to `subprocess.run`—especially the executable itself—must be validated against an allowlist of trusted paths to prevent command injection.
**Prevention:** Implement strict path validation (e.g., ensuring the name is "python" and the resolved path exists within safe directories like `/usr/bin` or virtual environments) before using it in subprocess execution.
