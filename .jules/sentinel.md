...
## 2026-08-11 - [Subprocess Hang Denial of Service (DoS) Vulnerability]
**Vulnerability:** Several utility and CI scripts utilized `subprocess.run()` to execute external commands (e.g. `git`, `gh`) without explicitly defining a `timeout`. If these external commands hang indefinitely due to network issues or unexpected interactive prompts, the subprocess will block the parent python process indefinitely, leading to resource exhaustion and potential Denial of Service (DoS) in CI/CD environments.
**Learning:** Python's `subprocess.run()` does not enforce a default timeout. It will wait indefinitely for the child process to terminate unless explicitly constrained. In automated environments, unbounded waits present a serious availability risk.
**Prevention:** Always provide an explicit `timeout` argument (e.g. `timeout=60`) to `subprocess.run()` calls to ensure fail-secure behavior (`subprocess.TimeoutExpired`) when an external command stalls.
