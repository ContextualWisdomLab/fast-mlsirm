...
## 2024-05-18 - [JSON Denial of Service (DoS) Vulnerability in Subprocess stdout]
**Vulnerability:** `scripts/build_pr_queue_governance.py`와 `scripts/build_procurement_due_diligence.py`에서 서브프로세스의 `stdout` 결과물에 대해 크기나 재귀 깊이 검증 없이 `json.loads`를 직접 사용했습니다. 악의적으로 크거나 깊게 중첩된 JSON 페이로드 반환 시 메모리 고갈 또는 재귀 한도 초과로 인한 서비스 거부(DoS)가 발생할 수 있습니다.
**Learning:** 파일 입력뿐만 아니라 외부 명령어(`gh` 등)의 출력 결과를 파싱할 때도 바운디드(bounded) JSON 파서를 사용하여 깊이와 크기를 제한해야 안전합니다.
**Prevention:** 서브프로세스의 출력을 역직렬화할 때는 항상 `scripts._bounded_json`의 `parse_json_bounded`를 사용하여 검증 후 `json.loads`가 수행되도록 합니다.
