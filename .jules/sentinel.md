...
## 2026-08-25 - [CSP unsafe-inline Risk & Dynamic Hashing]
**Vulnerability:** HTML reports generated by `fast_mlsirm` used `style-src 'unsafe-inline'` in their Content Security Policy (CSP). This allows any injected styles to be executed by the browser, potentially enabling UI redressing attacks or exfiltration.
**Learning:** Hardcoding `'unsafe-inline'` for CSS (or JavaScript) undermines the core protection of CSP. A dynamic, cryptographic hash is required for embedded script/style content to restrict execution strictly to the intended markup. Crucially, CSP hashes must match the enclosed content exactly—down to the byte, including all whitespace—and must *exclude* the wrapping tags themselves (e.g. `<style>`). Joining HTML strings can inadvertently alter leading/trailing whitespace around the tag, breaking the hash match.
**Prevention:**
- Avoid `'unsafe-inline'` in CSP directives.
- Use dynamically computed hashes (`hashlib.sha256(content.encode('utf-8'))`) for inline resources.
- Ensure the HTML element is built as a single formatted string (e.g., `f"<style>{css}</style>"`) to prevent whitespace drifts during string joining, while strictly hashing only the inner `css` string.
