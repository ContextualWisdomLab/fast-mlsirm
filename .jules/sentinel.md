...
## 2026-07-20 - [Unbounded JSON Deserialization in report.py]
**Vulnerability:** In `python/fast_mlsirm/report.py`, `render_diagnostics_report()` used `json.loads(source.read_text())` which read the entire file into memory and parsed it without size or nesting constraints. This exposed the application to DoS attacks via extremely large files (OOM) or deeply nested JSON (RecursionError/stack overflow).
**Learning:** Directly using standard library `json.loads(Path.read_text())` for untrusted or user-provided files bypasses central safety constraints and limits. Always use project-specific, bounded functions for deserializing data from files.
**Prevention:** Replaced raw `json.loads(source.read_text())` with the centralized `_load_json_bounded()` function from `fast_mlsirm.io`, which enforces `MAX_JSON_INPUT_BYTES` (32MB) and `MAX_JSON_NESTING_DEPTH` (128).
