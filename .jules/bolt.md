...
**Action:** Replace `(x * x).sum(axis=1)` with `np.einsum('ij,ij->i', x, x, optimize=True)` when computing pairwise Euclidean distances to avoid allocating the intermediate 2D array and achieve measurable performance gains.
