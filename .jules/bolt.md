...
## 2024-05-19 - Safe Refactoring and Vectorization
**Learning:** Refactoring formulas for performance using vectorization (e.g. `(a-b)^2 = a^2+b^2-2ab`) in complex numerical calculations like pairwise Euclidean distance can violate `AGENTS.md` guidelines if they piecemeal renovate model paths without updating tests/docs together, or if they accidentally remove imports or logic when bulk fixing `ruff` violations during cleanup.
**Action:** Opt for simple `np.vdot(x, x)` replacements instead of `np.sum(x * x)` in places like normalization routines to achieve safer performance gains with less risk of regressions or violating contribution guidelines. Avoid doing broad `ruff --fix` operations on unrelated files to keep PRs focused.
