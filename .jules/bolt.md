...
## 2025-05-19 - Replacing element-wise multiplication and sum with BLAS Matmul
**Learning:** Operations like `(e * params.theta[:, factors]).sum(axis=0) * a` perform an element-wise multiplication resulting in an intermediate N x J array, and then reduce it over axis 0. The same is true for `(posterior * nodes[None, :]).sum(axis=1)` which creates an N x Q intermediate array. Both of these operations are mathematically equivalent to matrix multiplications.
**Action:** Replace `(A * B).sum(axis=0)` and `(A * B).sum(axis=1)` patterns with matrix multiplications (`@`) whenever possible. For example, `(e.T @ params.theta)[np.arange(J), factors]` and `posterior @ nodes`. BLAS-backed matrix multiplications are heavily optimized, avoid O(N*M) intermediate array allocations, and can result in 10x-20x performance improvements.
