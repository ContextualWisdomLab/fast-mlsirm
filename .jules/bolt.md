...
## 2024-08-20 - Mathematical Simplification and Float Precision
**Learning:** While the algebraic identity `log_sigmoid(eta) = eta + log_sigmoid(-eta)` is mathematically correct, in finite precision arithmetic (like `float64`), evaluating `eta + log_sigmoid(-eta)` for large positive `eta` (e.g., `eta = 40`) results in `40 + (-40.0) = 0.0`. However, the direct `_log_sigmoid(eta)` (which wraps `-np.logaddexp(0.0, -eta)`) evaluates to approximately `-4.248e-18`. Replacing the evaluation can erase small negative log-likelihood contributions for extreme positive logits and alter Rust/reference parity.
**Action:** Do not substitute `log_sigmoid(eta)` with `eta + log_sigmoid(-eta)` for numerical optimization. When suggesting any mathematical transformation of transcendental functions for performance, ensure extreme-logit numerical equivalence against existing formulas and backend implementations, and always measure against a benchmark before changing reference estimators.
