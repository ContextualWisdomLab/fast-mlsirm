...
## 2024-05-19 - Fast Distance Calculation in Multidimensional Latent Spaces
**Learning:** Computing pairwise distances between points via `diff = xi[:, None, :] - zeta[None, :, :]; np.sum(diff * diff, axis=2)` leads to excessive intermediate allocations of shape `(N, J, D)`, consuming substantial memory and causing CPU cache misses when numbers of factors or dimensionalities are large.
**Action:** Replace intermediate large multi-dimensional matrices and subsequent axis reductions with operations like `np.einsum('nd,nd->n', ...)` combined with `np.dot` in order to formulate identical computations through efficient dot products that rely directly on optimized BLAS subroutines.
