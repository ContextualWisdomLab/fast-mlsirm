...
## 2025-05-19 - Intermediate allocations in distance calculations
**Learning:** `np.sum(diff * diff, axis=1)` in distance formulas creates an unnecessary intermediate 2D array before performing the sum over the axis. This memory allocation overhead is a performance bottleneck in inner optimization loops for large latent spaces.
**Action:** Replace `(x * x).sum(axis=1)` and `np.sum(x * x, axis=1)` with `np.einsum('ij,ij->i', x, x)` when computing pairwise distances to skip intermediate array allocations and improve execution time.
