...
## 2026-07-01 - NumPy Boolean Array Operations

**Learning:** When checking if any row or column in a boolean NumPy array is entirely empty/missing, using `.any()` combined with `.all()` (e.g., `not np.all(observed.any(axis=0))`) is significantly faster than using `.sum() == 0` (e.g., `np.any(observed.sum(axis=0) == 0)`). The `.sum()` method forces NumPy to cast the boolean array to integers and perform arithmetic addition across the entire axis, which is slower and consumes more memory.
**Action:** Always prefer `.any()` and `.all()` for logical reductions on boolean arrays instead of using `.sum()` to count `True` values, unless the actual count is needed.
