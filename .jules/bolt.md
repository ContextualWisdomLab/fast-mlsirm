...
## 2024-05-23 - Optimize distance computation with np.einsum
**Learning:** Using `np.sum(diff * diff, axis=1)` creates an intermediate array for `diff * diff` before summing, which causes unnecessary memory allocation and performance degradation, especially in hot loops or large array operations. `np.einsum('ij,ij->i', diff, diff)` performs the squaring and summation in a single pass without allocating a large intermediate array.
**Action:** When calculating row-wise sums of squared differences, use `np.einsum('ij,ij->i', diff, diff)` instead of `np.sum(diff * diff, axis=1)` to avoid intermediate array allocations and improve performance.
