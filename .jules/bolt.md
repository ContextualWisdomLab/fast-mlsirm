...
## 2024-05-18 - Avoid O(N*J*D) broadcasting for pairwise distance
**Learning:** When calculating pairwise Euclidean distances between vectors (e.g., between N persons and J items in D dimensions), using 3D broadcasting like `np.sqrt(((xi[:, None, :] - zeta[None, :, :]) ** 2).sum(axis=2))` incurs significant intermediate memory allocation overhead (O(N*J*D)) and is very slow.
**Action:** Use the 2D dot product trick `np.maximum(xi_sq[:, None] + zeta_sq[None, :] - 2 * np.dot(xi, zeta.T), 0.0)` which requires significantly less memory allocation and leverages BLAS operations (dot product), yielding ~6x speedup.
