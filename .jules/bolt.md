...
## 2024-05-19 - Boolean Array Checks Performance
**Learning:** `np.any(observed.sum(axis=...) == 0)` and `train.sum(axis=...) == 0` incur significant overhead due to boolean to integer casting and lack of short-circuiting.
**Action:** Replace `sum() == 0` checks with `.any()` boolean checks like `not np.all(observed.any(axis=...))` or `~train.any(axis=...)` for a roughly 15-20x performance improvement in array missingness/zero-count checks.
