...
## 2024-05-18 - Math simplifies Expected Binomial Log-Likelihood over Log-Sigmoids
**Learning:** In expected-count log-likelihood calculations like `r * _log_sigmoid(x) + (n - r) * _log_sigmoid(-x)` inside inner M-step optimization loops, evaluating the transcendental function `_log_sigmoid` (which wraps `np.logaddexp`) twice per array element adds significant overhead.
**Action:** Replace it with the mathematically equivalent `r * x + n * _log_sigmoid(-x)` by applying the identity `_log_sigmoid(x) - _log_sigmoid(-x) = x`. This halves the number of expensive log-sum-exp evaluations, yielding ~2x performance improvements for these specific lines without allocating intermediate arrays or altering the mathematical outcome.
## 2024-05-19 - Reference Implementation Parity and Floating-Point Cancellation Risk
**Learning:** In expected-count log-likelihood calculations, `r * _log_sigmoid(x) + (n - r) * _log_sigmoid(-x)` ensures numerical stability. The mathematical simplification `r * x + n * _log_sigmoid(-x)` is exact algebraically, but when $x$ (i.e. `eta`) is large, the terms `r * x` and `n * _log_sigmoid(-x)` (which behaves like $-nx$) become large with opposite signs. When $r \approx n$, this creates severe floating-point cancellation, rounding a small negative log-likelihood to exactly `0.0`. This undermines numerical stability on the reference pathway.
**Action:** Do not substitute mathematically "equivalent" forms that introduce catastrophic cancellation at extreme logit values. In `marginal.py`—which serves as the explicit NumPy reference implementation against which production Rust code is tested—numerical conservatism and auditable parity must be prioritized over minor runtime speedups.
