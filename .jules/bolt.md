...
## 2025-02-14 - Optimize expected Bernoulli log-likelihood via math identity & vdot
**Learning:** The expression `np.sum(r * _log_sigmoid(x) + (n - r) * _log_sigmoid(-x))` allocates large intermediate arrays and evaluates the expensive `_log_sigmoid` twice. It can be algebraically simplified to `r * x + n * _log_sigmoid(-x)` because `_log_sigmoid(x) - _log_sigmoid(-x) = x`. Moreover, when accumulating over multidimensional arrays in EM steps, replacing `np.sum(A * B + C * D)` with `np.vdot(A.ravel(), B.ravel()) + np.vdot(C.ravel(), D.ravel())` eliminates intermediate array allocations and massively boosts performance by relying on underlying BLAS calls.
**Action:** Always search for algebraic simplifications involving `log_sigmoid` or `logaddexp` first. In performance-critical M-step accumulations over quadrature dimensions, aggressively replace `np.sum(elementwise_product)` with flattened `np.vdot` to prevent memory blowups and accelerate scalar reductions.
