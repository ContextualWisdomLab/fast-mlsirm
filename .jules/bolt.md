...
**Learning:** Calculating multiple moments over the same Rust slice with separate iterator reductions performs two traversals. Fusing the reductions into one `fold` preserves linear O(N) complexity while reducing the number of slice traversals from two to one; the actual runtime effect depends on compiler optimization, cache state, input size, and the surrounding hot path. Rust documents `f64::powi` as having unspecified precision, so replacing a `powi(4)` call with explicit multiplications preserves the mathematical fourth-power formula but must not be described as bitwise numerical parity across Rust versions or platforms without exact-head evidence.
**Action:** Fuse compatible reductions only when the mathematical formula, finite/error contract, and validated tolerance behavior are retained. Treat changes between unspecified-precision helpers and explicit IEEE-754 arithmetic as numerical-contract changes that require focused parity/recovery evidence. Do not describe two O(N) traversals as an O(2N) to O(N) complexity-class improvement or claim a speedup percentage without a reproducible current-head benchmark. Rust reference: https://doc.rust-lang.org/std/primitive.f64.html#method.powi
