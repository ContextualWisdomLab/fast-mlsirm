...
## 2025-05-19 - Pre-reduction to avoid massive broadcasting
**Learning:** During covariate slope gradient calculation (`grad_d` and `info_d`), operations like `(resid * w_bcast).sum()` where `w_bcast` is `(S, I, 1, 1)` broadcasted to `(S, I, Qt, Nx)` cause severe memory overhead by materializing a 4D array just for scalar reduction.
**Action:** Always reduce (`.sum()`) over the inner axes (`axis=(2, 3)`) *first* so the array is shrunk to the broadcast dimensions, and then use `np.vdot` with the unbroadcasted arrays (`resid_sum` and `w_cov`). This eliminates the massive intermediate array allocation.
