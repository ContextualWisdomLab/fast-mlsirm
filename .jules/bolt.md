...
## 2025-05-19 - Simplifying transcendental functions for performance
**Learning:** In performance-critical mathematical code, calculating expected binomial log-likelihood via `np.sum(r * _log_sigmoid(x) + (n - r) * _log_sigmoid(-x))` involves two separate evaluations of the computationally expensive `_log_sigmoid` (which wraps `np.logaddexp`). Using the algebraic identity `_log_sigmoid(x) - _log_sigmoid(-x) = x`, the expression can be mathematically simplified to `np.sum(r * x + n * _log_sigmoid(-x))`, halving the number of transcendental function evaluations.
**Action:** When computing log-likelihoods or loss functions in loops or large array operations, mathematically simplify expressions to minimize evaluations of transcendental functions (like log, exp, sigmoid, logaddexp) before resorting to lower-level implementation optimizations.
