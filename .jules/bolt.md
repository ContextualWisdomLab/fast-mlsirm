...
## 2026-08-08 - Matrix-vector reductions for EAP theta calculations
**Learning:** In `fast_mlsirm/estimators/mmle.py`, the calculation `theta = (posterior * nodes[None, :]).sum(axis=1)` allocates a temporary `(N, Q)` broadcast array before summing over the nodes axis. This intermediate memory allocation scales poorly for large datasets and is numerically identical to a standard matrix-vector dot product.
**Action:** Replace broadcasting and axis summation with dense matrix multiplication `theta = posterior @ nodes` when aggregating probabilities over quadrature nodes, to eliminate intermediate allocations and leverage fast BLAS routines.
