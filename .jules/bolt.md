...
## 2024-05-19 - Intermediate allocations in distance calculations (marginal estimator)
**Learning:** `np.sum(diff * diff, axis=1)` creates an unnecessary intermediate 2D array before performing the sum over the axis. This causes a significant performance bottleneck when calculating distances over a grid of points (e.g. NxK arrays).
**Action:** Replace `np.sum(diff * diff, axis=1)` with `np.einsum('ij,ij->i', diff, diff)` to avoid allocating the intermediate 2D array and achieve measurable performance gains.
