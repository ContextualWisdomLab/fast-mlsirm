...
## 2024-05-19 - Categorical array reduction via boolean mapping and matrix multiplication
**Learning:** In EM steps, computing expected counts per category via `np.stack([post[y[:, i] == k].sum(axis=0) ...])` creates intermediate boolean arrays and performs unvectorized row-wise summations across each item `i` and category `k`, resulting in excessive memory allocation overhead and slow execution. However, allocating a giant intermediate array for all items, dimensions, and categories (e.g. `(J, Q, K)`) outside the loop causes a DoS memory allocation vulnerability if the test form is large.
**Action:** Replace `post[y == k].sum(axis=0)` loops inside the item loop with a safe, per-item matrix multiplication. By generating a boolean mask for the current item `(y[:, i, None] == np.arange(k_cat)).astype(post.dtype, copy=False)` and utilizing matrix multiplication `post.T @ mask`, the operations leverage highly optimized BLAS linear algebra routines while constraining peak memory to a small `(N, K)` slice.
