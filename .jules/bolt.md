...
## 2025-05-19 - Fast vectorized scalar reduction for element-wise array products
**Learning:** In NumPy, calculating a scalar sum of an element-wise multiplication between two arrays using `np.sum(A * B)` or `(A * B).sum()` creates an unnecessary intermediate array allocation in memory for the product before performing the sum. In large, iterative hot paths like EM algorithms or Computerized Adaptive Testing (CAT) updates, this intermediate allocation severely slows down performance and increases memory overhead.
**Action:** Replace `np.sum(A * B)` patterns with `np.vdot(A, B)` when a scalar reduction is needed over element-wise multiplied real-valued arrays of the same shape. This flattens the arrays and uses highly optimized C-level BLAS routines directly, entirely skipping the intermediate array allocation.

## 2025-05-19 - Replacing intermediate distance arrays with einsum
**Learning:** When calculating pairwise differences that are immediately squared and summed across an axis, like `np.sum(diff * diff, axis=1)` or `np.sum(diffz * diffz, axis=2)`, NumPy allocates large intermediate 2D or 3D arrays to store the results of the element-wise multiplication before summing.
**Action:** Replace `np.sum(A * A, axis=1)` with `np.einsum('ij,ij->i', A, A)` and `np.sum(B * B, axis=2)` with `np.einsum('ijk,ijk->ij', B, B)` to compute the squared sum across specific axes without allocating the intermediate arrays.
