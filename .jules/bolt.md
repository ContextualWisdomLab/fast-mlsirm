...
## 2025-05-19 - Intermediate allocations in distance calculations (einsum)
**Learning:** `(diff * diff).sum(axis=1)` in Euclidean distance formulas creates an unnecessary intermediate 2D array before performing the sum over the axis. This can cause performance bottlenecks when called repeatedly.
**Action:** Replace `(diff * diff).sum(axis=1)` with `np.einsum('ij,ij->i', diff, diff)` when computing pairwise Euclidean distances to avoid allocating the intermediate 2D array and achieve measurable performance gains.
