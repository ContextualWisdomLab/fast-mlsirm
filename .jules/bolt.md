...
**Learning:** In heavily mathematical procedures like MMLE where internal algorithms iterate per-item updating equations via Newton-Raphson, performing sequential Python loops (e.g., `for i in range(n_items)`) becomes a major performance bottleneck due to Python overhead, even when the inner steps are vectorized per node.
**Action:** When implementing or optimizing iterative algorithms that act independently across multiple dimensions (e.g., items), maintain an active boolean mask (e.g., `active = np.ones(n_items, dtype=bool)`) and process all active/unconverged elements simultaneously using 2D broadcasted arrays. This avoids the Python loop overhead and can result in order-of-magnitude speedups (e.g., MMLE estimation time dropped from ~9.3s to ~2.0s).
