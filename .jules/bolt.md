...
## 2025-06-25 - Avoid array allocation via matrix multiplication in numpy fallbacks
**Learning:** During the fallback estimation in `fast_mlsirm/estimators/mmle.py`, element-wise multiplication combined with `sum()` reductions (e.g. `(posterior * nodes[None, :]).sum(axis=1)`) results in allocating intermediate `(N, Q)` sized arrays. The same logic is mathematically equivalent to `posterior @ nodes`, which takes advantage of underlying BLAS matrix multiplication and avoids these costly allocations. This optimization is particularly beneficial across quadrature nodes or response patterns.
**Action:** Replace `(A * B[None, :]).sum(axis=1)` with matrix-vector multiplications `A @ B` (or `A @ np.ones(B.shape[0])` for simple row sums where equivalent). Furthermore, when constructing pairwise combination matrices, replace expressions like `A[:, None] * B[None, :]` with `np.outer(A, B)` or `A[:, None] * B` where appropriate, avoiding large broadcast allocations.
