...
## 2025-02-28 - Optimize binomial log-likelihood in NumPy
**Learning:** In heavily used optimization functions involving probability distributions, applying algebraic simplifications to transcendential function calls (like `_log_sigmoid`) is crucial. The expected binomial log-likelihood `r * _log_sigmoid(x) + (n - r) * _log_sigmoid(-x)` can be simplified to `r * x + n * _log_sigmoid(-x)` by leveraging the mathematical identity `_log_sigmoid(x) - _log_sigmoid(-x) = x`. This halves the number of expensive `np.logaddexp` computations within inner performance-critical loops (e.g., M-step).
**Action:** Always search for algebraic shortcuts when evaluating combinations of complimentary probabilities (e.g., successes vs. failures, log(p) and log(1-p)) before resorting to brute-force NumPy array evaluations.
