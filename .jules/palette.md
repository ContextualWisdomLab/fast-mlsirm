...
## 2026-07-18 - Improve Table Accessibility and Alignment
**Learning:** When generating HTML data tables, using `<th scope="row">` for the first identifying column improves screen reader accessibility. Additionally, applying `font-variant-numeric: tabular-nums;` to table cells ensures numbers align properly in columns.
**Action:** Use `<th scope="row">` for row headers and `font-variant-numeric: tabular-nums` for numeric data cells in tabular data representations.
