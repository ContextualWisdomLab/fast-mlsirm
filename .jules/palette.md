...
## 2025-02-12 - HTML Native Title Attributes
**Learning:** A proposed micro-UX improvement to expose full-precision floats using the native HTML `title` attribute was rejected because it is pointer-hover dependent, inconsistently exposed to assistive technologies, absent from touch and print workflows, and cannot provide semantic table/export relationships. Essential numeric evidence must remain available without hover, JavaScript, or a specific browser implementation.
**Action:** Do not rely on HTML `title` attributes to expose essential data. Instead, implement shared exact-value tables or disclosures that support keyboard/touch/print workflows and semantic relationships.
