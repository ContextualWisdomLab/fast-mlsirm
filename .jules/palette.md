...
**Learning:** Static HTML data reports are often printed or exported to PDF, but default browser settings strip background colors, which completely hides CSS-based data visualizations or grouping, while exposing unwanted interactive elements like skip links.
**Action:** Use `@media print` in CSS to optimize HTML reports for printing by hiding skip-links (`.skip-link { display: none !important; }`), ensuring correct background colors, and preventing awkward page breaks inside sections and tables (`break-inside: avoid`).
