...
## 2025-02-12 - Print Optimization for HTML Reports
**Learning:** Static HTML data reports are often printed or exported to PDF, where interactive navigation is unnecessary and preserving screen-only backgrounds can waste ink or produce inconsistent output across browsers.
**Action:** Use `@media print` in CSS to render a deliberate black-on-white body, hide skip links (`.skip-link { display: none !important; }`), and prevent awkward page breaks inside sections and tables (`break-inside: avoid`). Do not claim that screen backgrounds are preserved unless the CSS explicitly opts into print color adjustment and background rules.
