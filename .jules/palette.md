...
## 2024-11-20 - [CSS hover isolation accessibility regression]
**Learning:** Adding CSS hover isolation (`opacity: 0.5` for unhovered rows) in dense data tables breaks accessibility constraints by relying exclusively on pointer input. It weakens contrast for essential audit values and does not gracefully degrade for keyboard, touch, or print formats, which violates strict accessibility and report-contract constraints.
**Action:** Do not implement opacity-based dimming for unhovered table rows. Ensure all UI enhancements pass comprehensive accessibility checks including keyboard focus, contrast limits, forced colors, and print availability without relying on pointer-only interactions.
