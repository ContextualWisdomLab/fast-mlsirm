...
## 2025-01-20 - Skip-to-Content Target Focus Visibility
**Learning:** While `tabindex="-1"` and `outline: none;` on a skip-link target (`<main>`) prevents visual artifacts from mouse clicks, it also removes the visible focus indicator for keyboard users who rely on the skip-link to navigate. This makes it impossible for them to visually confirm that their focus has successfully moved to the main content area.
**Action:** Always pair `:focus { outline: none; }` with `:focus-visible { outline: [visible style]; }` (e.g. `outline: 3px solid #0f766e; outline-offset: 3px;`) on programmatically focusable skip-link targets. This ensures keyboard users receive a clear visual focus indicator without affecting mouse users.
